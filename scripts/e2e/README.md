...
| `suspend` | Suspend of the worker: Sandbox `Suspended`, pod gone, tree PVC `Bound`, `Probe(recorded)` gone; after Suspend returns Observe delivers no gone or not-recorded-process the supervisor's incarnation fence would pass (one carrying an incarnation the claim still holds). An observation that was already between its incarnation re-check and its send carries the suspended locator, which the fence drops; the check counts those, whatever their kind |

...
| `stop` | `legion claims stop` of the root architect is refused — 409, "the tree's root claim ends only when its tree closes; suspend it to stop its process" — and moves nothing; `legion claims suspend` suspends the root and `legion claims stop` retires the worker; `legion stop` ends the daemon with exit 0 |

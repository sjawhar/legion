# Live proofs: the stage gates of the Go coordinator

Each script here is one stage's gate: it runs the real binary against a real Postgres on this box
and fails loudly on the first step that does not hold. Unit tests do not gate a stage; these do.
A later stage's script lands beside this one.

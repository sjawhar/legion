...
| `SMOKE_UPSTREAM_NATS` | — (required with `SMOKE_GITHUB_INGRESS=envoy`) | the production Envoy NATS the bridge subscribes on (never publishes to), by its fully-qualified name on the operator's tailnet, `nats://envoy-nats.<tailnet>.ts.net:4222` — never a bare alias, which only a resolver's search domain completes ([the rig-alias learning](../../docs/solutions/testing/a-rig-container-alias-that-is-momentarily-unheld-resolves-through-the-tailnet-to-production.md)); `up.sh` refuses an envoy run without it, or with a value that is not one NATS URL naming a host with a dot (a path, query or second URL after the host included), before anything starts, and prints the variable's name, never its value |

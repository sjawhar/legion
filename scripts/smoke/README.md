...
After the `.legion/` deletion push, retro, and reviewer approval complete, run checkpoint 7 **before** asking for the human merge approval. It captures `main`'s base SHA under `SMOKE_DIR`; checkpoint 8 consumes that recorded value to prove the resulting merge is a squash onto that base.
...
| 5 | `SMOKE_IMPLEMENTER_LOGIN`, `SMOKE_REVIEWER_LOGIN`, `SMOKE_PR` optional | The pull request's head branch is `legion/<KEY>`, and every commit on it is both authored and committed by one of the two Legion GitHub App bots (`SMOKE_IMPLEMENTER_LOGIN`, the code-writing App, and `SMOKE_REVIEWER_LOGIN`, the review App) with that App's `<id>+<login>@users.noreply.github.com` address, at least one commit was committed by the code-writing App, and every commit message carries an `Omp-Session: <uuid>` trailer naming the committing pane's own session. A commit that misses fails naming its SHA, the field, and the value found; an empty commit list, or a full first page of 100, fails naming the count instead of passing on nothing. |
...
| 7 | `SMOKE_BRANCH_PROTECTION=1`, `SMOKE_PR`, `SMOKE_RETRO_COMMIT`, `SMOKE_IMPLEMENTER_LOGIN`, `SMOKE_REVIEWER_LOGIN` | The commit that removes `.legion/` was made by a Legion App — its author and committer are each one of the two bot logins (usually the code-writing App; the review App may make it locally in the shared workspace and the code-writing App's push carries it, since the review App holds no `contents` permission) — the reviewer's approval was submitted after that commit (every review page is read), the retro commit touches `docs/solutions/`, and the final PR diff has no `.legion` path; records the pre-merge base for checkpoint 8. |

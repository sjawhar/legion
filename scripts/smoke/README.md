...
In `isolated` mode `up.sh` starts `scripts/smoke/issue-relay.ts` after the root issue is recorded (the relay replays that issue's whole history from `GET /api/v1/issues/<key>/events`, so `issue.created` still arrives) and waits for its `RELAY READY` line before printing `RIG READY`. Every two seconds the relay discovers the root's transitive children through `GET /api/v1/issues?project=LEGSMOKE&parent=<key>`, fetches each tracked key's rows past the last acknowledged `seq`, and publishes them in global event-id order onto `notifications.dispatch.issue.<key>.<type>` in the rig's isolated NATS as the exact envelope the production Dispatch outbox emits (`event_id`/`dedupe_key`/`trace_id` `dispatch-<id>`, the whole event as `payload`), with the outbox's JetStream message id (`dispatch-<id>:<subject>`) so a replay after a relay restart is a duplicate the stream discards. Nothing else reaches the rig: no other rig's issues and no GitHub event. A Dispatch outage is retried with backoff (1 s doubling to 30 s) without skipping a row; a rejected bearer or a root issue Dispatch does not know stops the relay (`RELAY UNHEALTHY`, exit 1) and `up.sh` with it. `envoy` remains available for live GitHub ingress.

...
| 4 | `SMOKE_ROOT_ISSUE` optional | The tree moved: a released child is never a tree or admission entry of its own, and a child Dispatch reports at `in_progress` or later holds an architect (sub-architect) role claim whose locator is a worker pane (LEGION-57) — or a phase worker is claimed on the root (single-issue tree; the output names which); under `root-issues`, the human's approval opened the gate first (`approvedVersion == latestVersion`). |

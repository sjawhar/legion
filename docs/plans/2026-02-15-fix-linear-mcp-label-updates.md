...
2. Matches by name (exact first, then partial)
...
```
Label matching is fuzzy: "bug" → "Bug", "work" → "worker-active"
...
Run: `grep -c "resolveLabels\|labelIds\|getWorkspaceLabels" mcp/dist/index.js`
...
to IDs via fuzzy matching (exact then partial)."
```

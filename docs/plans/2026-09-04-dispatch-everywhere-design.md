...
hooks. The older per-repo App (`<github-app-slug>`) stays in active use for other purposes.
Belongs with a Legion-running session where the daemon's event path can be verified end to
end.

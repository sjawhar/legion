---
title: "The Claude plugin's committed bundles inline @legion/contracts and @legion/envoy-client: every change to either package carries a rebuilt dist/ commit, built from a tree that has main's plugin sources, on the pinned Bun"
category: legion
tags:
  - claude-envoy
  - check-dist
...
# Committed plugin bundles inline the contracts: every contracts change carries a dist/ commit
...
no `node_modules`; `packages/claude-envoy/AGENTS.md` says so and states the rule: rebuild
...
branches in flight conflict with each other on the bundle files. Whoever owns the plugin decides
...
can only be built from a tree that carries `main`'s plugin sources, and it costs one CI run either
...
- **One dist commit per branch.** A later rebase (LEGION-131's second, conflict-forced by #961)
  ...
  `jj -R "$LEGION_WORKSPACE" squash --into <that commit's change id> packages/claude-envoy/dist` —
...
- **`main` moves again while you work** (the shared clone's `main@origin` advanced twice under this
  ...
  packages/envoy-client packages/claude-envoy`. Empty means the bundles you built are
...
computed the skill's hash with `packages/claude-envoy-bridge/dist` (today
`packages/claude-envoy/dist`) added to the exclusion —
...
- **Implementer:** before opening or re-pushing a PR that touches `packages/contracts` or
  `packages/envoy-client`, run `bun run check-dist` in `packages/claude-envoy` on your tree; if it
  is stale, your branch needs the dist commit (and, if it predates #1103, the rebase above). Say in
  the PR body that the dist commit exists and why.

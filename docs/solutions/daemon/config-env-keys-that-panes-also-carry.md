...
- the retired rig's `legion.yaml`, generated by its former startup script, set
...
- The retired rig honoured the same inherited `LEGION_OMP_PATH` (LEGION-40): it treated the
  variable as the operator's explicit override of the pinned OMP, so inheritance silently changed
  the build it ran. The first LEGION-40 run printed

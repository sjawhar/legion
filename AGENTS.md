...
legion worker-shim --connect tcp://<host>:<port> --boot-token-file <path> [--provider-env-dir <dir>] -- <omp argv…>  # Same bridge, reverse-dialed: the shim dials the daemon's worker stream listener and authenticates with its boot token; --provider-env-dir exports each mounted secret file as NAME=contents into the OMP child's environment only (skipping a NAME the pod already consumes through a NAME_FILE pointer, e.g. DISPATCH_TOKEN; and refusing to start — exit 1 naming the key and its file, Oh My Pi never spawned — when a key's name is already a variable of the shim's own environment, since the export would override it silently (LEGION-186)); the shim also answers the daemon's `adopt-working-copy` frame by running the shared `jj metaedit --update-author` in its workspace; the Go shim, when LEGION_MODEL_GATEWAY_URL is set (a Sandbox pod), first writes the Oh My Pi profile's model route through that gateway (`packages/daemon-go/internal/modelroute`), hands Oh My Pi the profile's pins (every other provider disabled, which also leaves a pod without a repository's `.cursor/` rules and MCP servers and without web search backends; sessions held to the gateway's aliases; no model fallback; remote compaction, memory backends, SQL session storage, image URLs and auto-QA pushes held off) as its last settings overlay (`PI_CONFIG_FILES`), which outranks a repository's `.omp/config.yml` for each of those settings, and holds Anthropic Foundry off (`CLAUDE_CODE_USE_FOUNDRY=0`) and Oh My Pi's config root at the image's (`PI_CONFIG_DIR=.omp`, which `OMP_CONFIG_DIR` mirrors onto) so a repository's `.env` cannot move the route, and refuses to start, naming the variable, on one it cannot route (Kubernetes runtime; daemon-spawned)

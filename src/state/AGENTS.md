...
| `cli.ts` | Entry point for pipe invocation: `echo $JSON | bun run src/state/cli.ts --team-id X --daemon-url Y`. Reads stdin, calls fetch + decision, writes JSON to stdout. |

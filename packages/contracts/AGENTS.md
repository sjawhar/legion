...
- `dispatch_ask` has no `kind` argument and `AskKind` is `"question" | "approval"`: every client-created ask is a question whose asker chooses its options (a human to-do is the to-do phrased as the question with `Done` / `Can't` options); `approval` remains server-created through `dispatch_request_approval`.

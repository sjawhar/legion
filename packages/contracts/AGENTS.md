...
| Go daemon API schema | `src/legion-go-api.ts`, `fixtures/daemon-api/` | Hand-written mirror of the Go daemon's wire shapes (`packages/daemon-go/internal/api/state.go`), which are the source of truth: no generator runs either way. `go test ./internal/api/ -update` writes the golden fixtures and `src/legion-go-api.test.ts` parses each one through the strict schema, so a Go field without its zod line fails there. Reached as `@legion/contracts/legion-go-api`, never from the barrel: at this stage nothing outside `packages/contracts` imports it — the plugin's daemon client is its reader from Stage 2 — and the barrel is bundled byte for byte into the Claude bridge, which would carry the schema for nothing. Every export is prefixed `LegionGo`/`GoDaemon` so both daemons' names read apart. `src/legion-daemon-api.ts` stays the TypeScript daemon's own contract until Stage 7 deletes it with `packages/daemon`. |

...
- `SearchResult.owner` is always present: an issue owner carries `key`, `title`, and `status`; a standalone project-document owner carries `project`, `slug`, `artifact_id`, and `name`. There is no separate issue block on a search hit, and a message hit's `href` is `/issues/KEY/conversation`. Ask event payloads carry `options` as an array, never `null`.

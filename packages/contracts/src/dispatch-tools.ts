...
export const dispatchToolSpecs = [
  ...
  },
  {
    ...
    description:
      ...
      "For replace, delete, and quote anchors, find text as rendered: inline Markdown (**bold**, `code`) is tolerated and must be balanced; a leading '# ' matches a heading. replace is inline: with is the new text of the matched span, so a marker of a different kind from the block's own stays literal text ('4. Design' written into a heading). A with that opens with a marker of the same kind as the matched block's own would write it twice and is INVALID_OP - including prose that merely looks like one ('1999. was a year' into an ordered item), which you write as text by escaping it ('1999\\. was a year'). The exception is a heading rename whose find carried the heading marker: replace(find=\"## Old\", with=\"## New\") gives '## New', and a different level there retitles and sets that level. " +
        ...
        .array(
          z.object({
            ...
            with: z
              .string()
              .describe(
                "Replacement text for replace, parsed as inline markdown within the matched block; a marker of a different kind from the block's own is literal text, one of the same kind is refused unless it is a heading rename (where a different level also sets that level), and a backslash escape keeps prose that merely looks like a marker."
              )
              .optional(),
            occurrence: z
              .number({ int: true, min: 0 })

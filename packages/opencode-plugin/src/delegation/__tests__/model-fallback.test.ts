...
describe("createModelFallbackChain", () => {
  it("returns a FallbackChain with the primary model and fallbacks", async () => {
    const { createModelFallbackChain } = await import("../../overlays");
    const chain = createModelFallbackChain("anthropic/claude-opus-4-6", [
      "anthropic/claude-sonnet-4-6",
      "openai/gpt-5.2-codex",
    ]);

    expect(chain.primary).toBe("anthropic/claude-opus-4-6");
    expect(chain.fallbacks).toEqual(["anthropic/claude-sonnet-4-6", "openai/gpt-5.2-codex"]);
  });

  it("returns empty fallbacks array when no fallbacks provided", async () => {
    const { createModelFallbackChain } = await import("../../overlays");
    const chain = createModelFallbackChain("anthropic/claude-opus-4-6");
  ...
  it("provides an iterator that yields primary first then fallbacks in order", async () => {
    const { createModelFallbackChain } = await import("../../overlays");
    const chain = createModelFallbackChain("anthropic/claude-opus-4-6", [
      "anthropic/claude-sonnet-4-6",
      "openai/gpt-5.2-codex",
  ...
  it("deduplicates models in the chain", async () => {
    const { createModelFallbackChain } = await import("../../overlays");
    const chain = createModelFallbackChain("anthropic/claude-opus-4-6", [
      "anthropic/claude-opus-4-6",
      "anthropic/claude-sonnet-4-6",
...
describe("createRetryWithFallback", () => {
  it("succeeds on first model when it works", async () => {
    const { createRetryWithFallback } = await import("../../delegation");
    const { createModelFallbackChain } = await import("../../overlays");

    const chain = createModelFallbackChain("model-a", ["model-b", "model-c"]);
    const attempts: string[] = [];

    const result = await createRetryWithFallback(chain, async (model) => {
      attempts.push(model);
  ...
  it("returns comprehensive error when all models exhausted", async () => {
    ...
    const promise = createRetryWithFallback(chain, async (model) => {
    ...
    try {
      await createRetryWithFallback(chain, async (model) => {
        throw new Error(`${model} failed`);
      });
    } catch (err) {
  ...
  it("single model failure throws original error", async () => {
    ...
    try {
      await createRetryWithFallback(chain, async () => {

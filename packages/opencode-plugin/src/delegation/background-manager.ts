...
import { executeWithModelFallback } from "./retry-with-fallback";
...
export class BackgroundTaskManager {
  ...
  private async startPrompt(task: BackgroundTask, opts: LaunchOptions): Promise<void> {
    try {
      ...
      const fallbackChain = createModelFallbackChain([task.model, ...(opts.fallbackModels ?? [])]);

      if (fallbackChain.fallbacks().length > 0) {
        const sessionID = task.sessionID;
        const { model: successModel } = await executeWithModelFallback(
          fallbackChain.models,
          async (modelStr) => this.sendPrompt(sessionID, modelStr, opts)
        );

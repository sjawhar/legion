...
describe("context_search", () => {
  ...
  it("rejects limit of 0", async () => {
    ...
    const tool = createContextSearchTool(store);
    const result = await tool.execute({ queries: ["test"], limit: 0 }, makeContext());

    expect(result).toContain("Error");
  });

  it("rejects limit greater than 10", async () => {
    const store = {
      search: () => [],
    } as unknown as ContentStore;

    const tool = createContextSearchTool(store);
    const result = await tool.execute({ queries: ["test"], limit: 11 }, makeContext());
  ...
  it("rejects empty queries array", async () => {
    ...
    const tool = createContextSearchTool(store);
    const result = await tool.execute({ queries: [] }, makeContext());

    expect(result).toContain("Error");
  });

  it("rejects queries array with more than 10 items", async () => {
    const store = {
      search: () => [],
    } as unknown as ContentStore;

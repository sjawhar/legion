...
export type JjCommandRunner = (
	rootDir: string,
	args: string[],
) => Promise<JjCommandResult>;

function defaultHotspotRunner(
	rootDir: string,
	args: string[],
): Promise<JjCommandResult> {
...
export async function buildChangeHotspots(
	...
	const historyLimit =
		options?.hotspotHistoryLimit ?? DEFAULT_HOTSPOT_HISTORY_LIMIT;
	...
	try {
		...
		if (result.exitCode !== 0) {
			options?.warn?.(
				`[index] Failed to compute hotspots from jj log: ${result.stderr.trim()}`,
			);
			return [];
		}
		...
		options?.warn?.(
			`[index] Failed to compute hotspots from jj log: ${String(error)}`,
		);
		return [];
	}
}

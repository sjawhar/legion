...
async function moveCorruptFile(filePath: string): Promise<void> {
	...
	try {
		...
		console.warn(
			`[index] Failed to move corrupt index file ${filePath}:`,
			error,
		);
	}
}
...
export async function readCodebaseIndex(
	...
	try {
		...
		try {
			...
			console.warn(
				`[index] Corrupt JSON in ${filePath}, moving aside and rebuilding`,
			);
		...
		if (!validation.success) {
			const issues = validation.error.issues
				...
				.join(", ");
			console.warn(
				`[index] Schema validation failed for ${filePath}: ${issues}`,
			);
			await moveCorruptFile(filePath);
			return null;
		}
...
export async function writeCodebaseIndex(
	filePath: string,
	index: CodebaseIndex,
): Promise<void> {

...
function buildTestMapping(
	...
	const sourceFiles = allRelativePaths.filter(
		(relativePath) => !isTestFile(relativePath),
	);
	const testFiles = allRelativePaths.filter((relativePath) =>
...
export async function updateDependencyGraphIncremental(
	...
	const testMapping = buildTestMapping(
		[...sourceByRelativePath.keys()],
		nextGraph,
	);

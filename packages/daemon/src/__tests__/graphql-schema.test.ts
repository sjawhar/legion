...
import {
	buildProjectStatusFieldQuery,
	buildStatusMutation,
} from "../state/backends/github";
...
const schema = buildSchema(
	gunzipSync(
		readFileSync(
			join(import.meta.dir, "..", "..", "schema", "github.docs.graphql.gz"),
		),
	).toString("utf8"),
	{ assumeValid: true },
);
...
describe("daemon GraphQL queries validate against GitHub's published schema", () => {
	...
	it("CI status batch query is schema-valid", async () => {
		const { runner, queries } = captureRunner(() =>
			ok({ data: { repo0: { pr0: null } } }),
		);
		await getCiStatusBatch(
			{ "acme-api-42": { owner: "acme", repo: "api", number: 101 } },
			runner,
		);
		expect(queries).toHaveLength(1);
	...
	it("issue-transition query and mutation are schema-valid", () => {
		expectValidQuery(
			buildProjectStatusFieldQuery("acme", "api", 42),

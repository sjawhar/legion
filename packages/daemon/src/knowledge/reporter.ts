...
export interface ConsolidationReportLike {
	...
	learnings: Array<
		Pick<ClassifiedLearningAggregate, "disposition" | "notes" | "path">
	>;
	legionId: string;
	logPath: string;
...
export function formatConsolidationReportHuman(
	report: ConsolidationReportLike,
): string {
	...
	const orderedLabels = ["PROMOTE", "REVIEW", "STALE", "KEEP"].filter((label) =>
		groups.has(label),
	);
	const lines = [
	...
	for (const label of orderedLabels) {
		...
		for (const learning of learningGroup.toSorted((left, right) =>
			...
			lines.push(
				`- ${learning.path}${learning.notes ? ` — ${learning.notes}` : ""}`,
			);
		}
	}
...
export function formatConsolidationReportJson(
	report: ConsolidationReportLike,
): string {

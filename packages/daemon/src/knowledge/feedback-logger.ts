...
export function deriveLegionIdFromWorkspaceDir(
	...
	if (
		...
		throw new Error(
			`Workspace is not inside configured workspaces dir: ${workspaceDir}`,
		);
	}
	...
	if (parts.length < 2) {
		throw new Error(
			`Workspace path does not include legion owner and issue: ${workspaceDir}`,
		);
	}
...
export function getLearningFeedbackLogPath(
	...
	return path.join(
		paths.forLegion(legionId).legionStateDir,
...
export async function appendLearningFeedbackRecord(
	...
	const filePath = getLearningFeedbackLogPath(
		options.legionId,
		options.env,
		options.homeDir,
	);
	const writer =
		options.writer ??
		new FileLearningFeedbackWriter(filePath, options.maxBytes);

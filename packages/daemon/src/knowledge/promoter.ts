...
export async function readAssembledIndex(
	indexDir: string,
...
export function deriveIndexPrefixes(
	filesChanged: string[],
	existingKeys: string[],
): string[] {
				...
				for (const key of normalizedKeys) {
					if (
						(normalizedFilePath === key ||
							normalizedFilePath.startsWith(`${key}/`)) &&
...
export async function trimToSoftCap(
	entries: string[],
	docsRoot: string,
): Promise<string[]> {
	...
	const rankedEntries = await Promise.all(
		entries.map(async (entry) => {
			const frontMatter = await readLearningFrontMatter(
				path.join(docsRoot, entry),
			).catch(() => null);
...
export async function applyPromotions(
	...
	for (const learning of promoted) {
		...
		const prefixes = deriveIndexPrefixes(
			learning.touchedPaths,
			Object.keys(assembledIndex.index),
		);
		for (const prefix of prefixes) {
			const currentEntries = assembledIndex.index[prefix] ?? [];
			const dedupedEntries = Array.from(
				new Set([...currentEntries, learning.path]),
			);
			...
			assembledIndex.index[prefix] = await trimToSoftCap(
				dedupedEntries,
				docsRoot,
			);
	...
	return {
		mutations: mutations.toSorted(
			(left, right) =>
				...
				left.learningPath.localeCompare(right.learningPath),
		),
		warnings: [],
	};
}

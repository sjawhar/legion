...
function canonicalizePhase(
	...
	const helpful = Array.from(
		new Set(phase.helpful.map(canonicalizeLearningPath)),
	).sort();
	const injected = Array.from(
		new Set(phase.injected.map(canonicalizeLearningPath)),
	).sort();
...
async function readLearningFeedbackLog(
	...
	const legionPaths = resolveLegionPaths(
		options.env,
		options.homeDir,
	).forLegion(options.legionId);
	const logPath = path.join(
		legionPaths.legionStateDir,
	...
	for (const [index, line] of fileContents.split(/\r?\n/).entries()) {
		...
		try {
			const parsedRecord = LearningFeedbackRecordSchema.parse(
			...
			warnings.push(
				`[knowledge] Malformed JSONL at line ${index + 1}: skipped`,
			);
		}
	}
...
function collectWorkspaceIssue(
	...
	for (const phase of HANDOFF_PHASES) {
		...
		if (
			phase === "implement" &&
			"filesChanged" in handoff &&
			Array.isArray(handoff.filesChanged)
		) {
...
export function dedupeCollectedIssues(
	issues: CollectedIssueCandidate[],
	...
	return Array.from(deduped.entries())
		.sort(([leftIssueId], [rightIssueId]) =>
			leftIssueId.localeCompare(rightIssueId),
		)
		.map(([issueId, entry]) => ({
			issueId,
			records: (entry.logRecords.length > 0
				? entry.logRecords
				: entry.workspaceRecords

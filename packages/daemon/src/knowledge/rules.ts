...
export function classifyLearningAggregate(
	...
	const isStale =
		...
		now.getTime() - lastInjectedAt > NINETY_DAYS_MS;
	...
	if (aggregate.helpfulRatio >= 0.7 && aggregate.issuesHelpful >= 3) {
		return {
			...
			notes:
				"Promote this learning because it is repeatedly helpful across issues.",
		};
	}
	...
	if (isStale && aggregate.issuesHelpful === 0) {
		return {
			...
			notes:
				"Archive this learning because it has gone stale without helpful confirmations.",
		};
	}
	...
	if (aggregate.issuesInjected >= 3 && aggregate.helpfulRatio < 0.3) {
		return {
			...
			notes:
				"Review this learning because it is frequently injected but rarely helpful.",
		};
	}

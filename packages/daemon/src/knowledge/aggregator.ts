...
const NON_ARCHITECT_PHASES = new Set<HandoffPhase>(
	KNOWLEDGE_NON_ARCHITECT_PHASES,
);
...
export function aggregateLearningFeedback(
	...
	for (const issue of issues) {
		for (const record of issue.records) {
			for (const [phase, phaseFeedback] of Object.entries(
				record.phases,
			) as Array<[HandoffPhase, (typeof record.phases)[HandoffPhase]]>) {
				if (!phaseFeedback) {
					continue;
				}
				...
				for (const helpfulPath of phaseFeedback.helpful) {
					...
					const helpfulIssue = ensureHelpfulIssue(
						aggregate,
						issue.issueId,
						record.timestamp,
					);
		...
		.map((aggregate) => {
			...
			return {
				...
				issues: aggregate.issueOrder
					...
					.filter(
						(issue): issue is HelpfulIssuePromotionContext => issue !== null,
					),
				issuesHelpful,
				issuesInjected,

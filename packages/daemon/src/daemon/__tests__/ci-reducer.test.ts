...
function resetForSynchronizedPush(
	pr: PrState,
	headSha: string,
	now: number,
): void {
...
describe("CI edge reducer", () => {
	...
	it("emits one complete settled-red batch after a randomized eight-observation burst", () => {
		...
		expect(emissions.map((emission) => emission.type)).toEqual([
			"ci-first-red",
			"ci-settled-red",
		]);
	...
	it("emits green for a new all-green head after synchronize resets the edge flags", () => {
		...
		expect(settle(pr, 400 + QUIET_MS, QUIET_MS)).toEqual([
			{ type: "ci-green", sha: "head-2" },
		]);
	});
	...
	it("emits nothing for queued and running transitions", () => {
		...
		reduceCheck(
			pr,
			{ sha: pr.headSha, name: "unit", status: "queued", conclusion: null },
			100,
		);
		reduceCheck(
			pr,

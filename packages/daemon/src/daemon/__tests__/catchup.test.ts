...
function stateForTree(): {
  ...
  state.trees[root] = { root, generation: 1, status: "active", launchFailures: 0, heldEvents: [] };
...
describe("derived catch-up", () => {
  ...
  it("includes held dispatch-thread replies for the resumed role", async () => {
    ...
    state.dispatchThreads.push({ repo: "acme/widgets", thread: 77, role, issue: root, tree: root });
    state.trees[root].heldEvents.push({
      role: token,

...
describe("daemon config", () => {
	...
	it("rejects missing NATS or dispatch configuration instead of inventing a transport", () => {
		...
		expect(() =>
			loadConfig({ LEGION_ID: "acme/7", ENVOY_NATS_URL: "nats://one:4222" }),
		).toThrow("LEGION_DISPATCH_URL");
	});

	it("rejects invalid lifecycle numbers from either configuration source", () => {
		expect(() =>
			loadConfig({ ...requiredEnv, LEGION_ADMISSION_CAP: "-1" }),
		).toThrow("LEGION_ADMISSION_CAP");
		expect(() =>
			loadConfigFromFile(
				"project: acme/7\nworker_budget: 1.5\n",
				"/tmp/legion-config",

...
describe("reduceGithubEvent", () => {
  ...
  it("creates an issue from a board item-created event", () => {
    ...
    expect(result).toEqual([
      { kind: "controller", payload: { type: "triage", issue: root, preexistingChildren: [] } },
    ]);
  });
  ...
  it("adopts a human-added child on an active tree but ignores an already-recorded legion child", () => {
    ...
    expect(
      effects(state, { action: "sub_issue_added", parent_issue: issue(1), sub_issue: issue(2) })
    ).toEqual([
      { kind: "publish", role: architect, payload: { type: "child-adopted", child, remaining: 1 } },
    ]);
    expect(state.issues[root].children).toEqual([child]);
    expect(state.issues[child]).toMatchObject({ parent: root, released: false });
  ...
  it("re-arms children-complete after reopening a child and fires it on the next close", () => {
    ...
    expect(effects(state, { action: "reopened", issue: issue(2) })).toEqual([
      { kind: "publish", role: architect, payload: { type: "child-reopened", child } },
    ]);
    expect(effects(state, { action: "closed", issue: issue(2, { state: "closed" }) })).toEqual([
      {
        kind: "publish",
  ...
  it("reports a removed child and emits children-complete when it was the last open child", () => {
    ...
    expect(
      effects(state, { action: "sub_issue_removed", parent_issue: issue(1), sub_issue: issue(2) })
    ).toEqual([
      { kind: "publish", role: architect, payload: { type: "child-removed", child, remaining: 0 } },
      { kind: "publish", role: architect, payload: { type: "children-complete" } },
    ]);
  ...
  it("routes a reopened removed child through controller triage instead of root resurrection", () => {
    ...
    expect(effects(state, { action: "reopened", issue: issue(2) })).toEqual([
      { kind: "controller", payload: { type: "triage", issue: child, preexistingChildren: [] } },
    ]);
  });

  it("starts linger when a root of an active tree closes", () => {
    const state = rootState();
    expect(effects(state, { action: "closed", issue: issue(1, { state: "closed" }) })).toEqual([
  ...
  it("routes plain issue comments to the issue architect", () => {
    ...
    expect(
      effects(state, {
        action: "created",
        issue: issue(1),
        comment: { user: { login: "sami" }, body: "Please adjust scope", html_url: "comment-url" },
      })
    ).toEqual([
      {
        kind: "publish",
    ...
    expect(
      effects(
        state,
        {
          action: "created",
          issue: issue(2),
          comment: { user: { login: "sami" }, body: "Hold this", html_url: "comment-url" },
        },
        undefined,
        "event-hold"
      )
    ).toEqual([
      {
        ...
        tree: root,
        role: childArchitect,
        payload: { type: "issue-comment", author: "sami", body: "Hold this", url: "comment-url" },
      },
    ]);
  ...
  it("filters legion-footer and self-authored comments before routing", () => {
    ...
    expect(
      effects(state, {
        action: "created",
        issue: issue(1),
        comment: { ...comment, user: { login: "legion-author[bot]" }, body: "Normal" },
      })
    ).toEqual([]);
  });
  ...
  it("routes registered dispatch-thread replies to their registered role", () => {
    const state = rootState();
    state.dispatchThreads.push({ repo, thread: 1, role: "architect", issue: root, tree: root });
    const architect = roleToken(state.project, root, "architect");
    ...
    expect(
      effects(state, {
        action: "created",
        issue: issue(1),
        comment: { user: { login: "sami" }, body: "Approved", html_url: "comment-url" },
      })
    ).toEqual([
      {
        kind: "publish",
  ...
  it("maps legion issue branches on PR opening and notifies the implementer", () => {
    ...
    expect(state.prs[`${repo}#${prNumber}`]).toMatchObject({ key: child, headSha: "head-sha" });
  });
  ...
  it("resets PR checks and approval state on synchronization, counts a red-head retry, and rechecks approval", () => {
    ...
    expect(
      effects(state, {
        action: "synchronize",
        pull_request: { number: prNumber, head: { ref: "legion/issue-2", sha: "new-sha" } },
      })
  ...
  it("treats neutral and skipped completed checks as green for an approved review", () => {
    ...
    expect(
      effects(state, {
        ...
        review: { user: { login: "sami" }, state: "approved", body: "Looks good" },
      })
    ).toEqual([
      {
        kind: "publish",
        role: implementer,
        payload: { type: "pr-review", state: "approved", author: "sami", body: "Looks good" },
      },
      { kind: "approval-status", repo, pr: prNumber, sha: "old-sha" },
      { kind: "publish", role: architect, payload: { type: "pr-ready", pr: prNumber } },
    ]);
  });
  ...
  it("counts completed checks missing a conclusion as red when a PR synchronizes", () => {
    const state = rootState();
    attachChild(state);
    addPr(state, { checks: { delayed: { status: "completed", conclusion: null } } });
  ...
  it("removes an unmerged PR mapping and tells the issue architect", () => {
    ...
    expect(
      effects(state, {
        action: "closed",
        pull_request: { number: prNumber, merged: false, head: { ref: "legion/issue-2" } },
      })
    ).toEqual([
      { kind: "publish", role: architect, payload: { type: "pr-closed-unmerged", pr: prNumber } },
    ]);
  ...
  it("records review state, notifies the implementer, emits approval-status, and notifies ready architect on a new green approval", () => {
    ...
    const implementer = claim(state, child, "implementer");
    const architect = claim(state, child, "architect");
    addPr(state, { checks: { tests: { status: "completed", conclusion: "success" } } });
  ...
  it("mirrors only surviving labels and ignores retired labels", () => {
    const state = rootState();
    expect(
    ...
    effects(state, { action: "labeled", issue: issue(1), label: { name: "worker-done" } });
    expect(state.issues[root].labels).toEqual(["needs-approval"]);
  ...
  it("indexes session attribution from a legion push trailer and comment footer", () => {
    ...
    expect(state.attribution).toEqual([
      { sha: "abc123", sessionId: "ses_push", issue: child, phase: "implement" },
    ]);
...
describe("reduceCiEmission", () => {
  it("notifies the architect when the CI edge turns green for an approved PR", () => {
    ...
    ).toEqual([{ kind: "publish", role: architect, payload: { type: "pr-ready", pr: prNumber } }]);
  });

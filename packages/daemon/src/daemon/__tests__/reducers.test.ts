...
function rootState(
	status: "active" | "lingering" | "closed" = "active",
): LegionState {
...
describe("reduceGithubEvent", () => {
	...
	it("filters legion-footer and self-authored comments before routing", () => {
		...
		expect(
			effects(state, { action: "created", issue: issue(1), comment }),
		).toEqual([]);
		expect(
			effects(state, {
				action: "created",
	...
	it("maps legion issue branches on PR opening and notifies the implementer", () => {
		...
		expect(state.prByBranch[`${repo}@legion/issue-2`]).toBe(
			`${repo}#${prNumber}`,
		);
	...
	it("resets PR checks and approval state on synchronization, counts a red-head retry, and rechecks approval", () => {
		...
		expect(
			...
			{ kind: "approval-status", repo, pr: prNumber, sha: "new-sha" },
		]);
		expect(state.prs[`${repo}#${prNumber}`]).toMatchObject({
			headSha: "new-sha",
			checks: {},
	...
	it("mirrors supported labels, waking the architect once when a human approves", () => {
		...
		expect(
			...
			{ kind: "publish", role: architect, payload: { type: "human-approved" } },
		]);
	...
	it("indexes session attribution from a legion push trailer and comment footer", () => {
		...
		expect(
			effects(state, {
				...
				commits: [
					{ id: "abc123", message: "Implement it\n\nLegion-Session: ses_push" },
				],
			}),
		).toEqual([]);

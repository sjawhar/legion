...
class FakeNats {
	...
	emit(subject: string, data: string): void {
		for (const subscription of this.subscriptions) {
			if (matches(subscription.subject, subject))
				subscription.callback(subject, data);
		}
	}
	...
	return {
		deps: {
			...
			runner: async (command) => {
				...
				if (controllerSecret)
					onControllerSecret(
						controllerSecret.slice("LEGION_CONTROLLER_SECRET=".length),
					);
...
describe("startDaemon", () => {
	it("runs board resync queries with the board owner's implementer App token", async () => {
		...
		try {
			const { createBoardProjectItemsFetcher } =
				daemonIndex as typeof daemonIndex & {
			...
			if (!createBoardProjectItemsFetcher)
				throw new Error("Missing board resync fetcher");
			...
			expect(tokenCalls).toEqual([
				{ role: "implement", owner: "trajectory-labs-pbc" },
			]);
	...
	it("heals missed board items and executes reconciled human approval wakes through the event pump", async () => {
		...
		const state = newLegionState(
			daemonConfig.project,
			daemonConfig.admissionCap,
		);
		const approvalIssue = formatIssueKey("acme", "widgets", 43);
		const architect = roleToken(
			daemonConfig.project,
			approvalIssue,
			"architect",
		);
	...
	it("persists a controller delivery exception across restart and redelivers it once when ready", async () => {
		...
		try {
			...
			const beforeReady = await fetch(
				`http://127.0.0.1:${second.server.port}/legion/v1/state`,
			);
	...
	it("boots the API, intake, and lifecycle timers when OMP emits its capability marker on stdout", async () => {
		...
		try {
			...
			const response = await fetch(
				`http://127.0.0.1:${daemon.server.port}/legion/v1/state`,
			);
			...
			expect(
				nats.subscriptions.map((subscription) => subscription.subject),
			).toEqual([
				...
				startDaemon(daemonConfig, {
					deps: {
						...
						createNatsTransport: async () => {
							natsCreated = true;
							throw new Error(
								"NATS must not start after a failed OMP capability probe",
							);
						},
					},
				}),

...
describe("TokenManager", () => {
	...
	it("caches a missing owner after the one allowed installation refresh", async () => {
		...
		await expect(
			manager.getToken("implement", "missing-owner"),
		).rejects.toThrow("github_app_not_installed");
		await expect(
			manager.getToken("implement", "MISSING-OWNER"),
		).rejects.toThrow("github_app_not_installed");
	...
	it("caches tokens per role and owner until the refresh window", async () => {
		...
		const fetchFn = mock(async (input: string | URL | Request) => {
			fetchCalls++;
			const installationId = String(input).match(
...
describe("buildRoleEnv", () => {
	it("scrubs sensitive env vars and adds role-specific ones", () => {
		...
		expect(result.GIT_AUTHOR_EMAIL).toBe(
			"111+legion-implement[bot]@users.noreply.github.com",
		);
		expect(result.GIT_COMMITTER_NAME).toBe("legion-implement[bot]");
		expect(result.GIT_COMMITTER_EMAIL).toBe(
			"111+legion-implement[bot]@users.noreply.github.com",
		);
	...
	it("scrubs LEGION_GITHUB_APP_* env vars containing inline app credentials", () => {
		...
		expect(
			result.LEGION_GITHUB_APP_IMPLEMENT_INSTALLATIONS_ACME,
		).toBeUndefined();

...
describe("modeToRole", () => {
  ...
  it("maps test, architect, plan to ops", () => {
...
describe("buildRoleEnv", () => {
  it("scrubs sensitive env vars and adds role-specific ones", () => {
    ...
    const result = buildRoleEnv("impl", "ghs_role_token", identity, baseEnv);

    // Scrubbed keys should be absent or replaced
    expect(result.HOME).toBe("/tmp/legion-impl");
  ...
  it("scrubs LEGION_GITHUB_APP_* env vars containing private key paths", () => {
    ...
    const result = buildRoleEnv("impl", "ghs_token", identity, baseEnv);

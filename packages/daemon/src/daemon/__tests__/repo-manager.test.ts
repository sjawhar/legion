...
describe("resolveWorkspacePath", () => {
	...
	it("throws when issueId would escape workspaces directory", () => {
		const paths = resolveLegionPaths({}, "/home/test");
		expect(() =>
			resolveWorkspacePath(paths, "sjawhar/42", "../../../etc"),
		).toThrow("Workspace path would escape workspaces directory");
	});
});
...
describe("ensureWorkspace", () => {
	it("creates jj workspace when it does not exist", async () => {
		...
		const wsPath = await ensureWorkspace(
			paths,
			"sjawhar/42",
			"acme-widgets-7",
			repo,
	...
	it("retries with force when workspace add fails with 'already registered'", async () => {
		...
		const forceAddCmd = gitCommands.find(
			(c) => c.includes("-f") && c.includes("add"),
		);
		expect(forceAddCmd).toBeDefined();

		// Should have called git worktree remove --force to clean up
		const removeCmd = gitCommands.find(
			(c) => c.includes("remove") && c.includes("--force"),
		);
			...
			runJj: async (args) => {
				if (args.includes("bookmark")) {
					return {
						...
						stdout: [
							"acme-widgets-7 local",
							"acme-widgets-7 remote:origin ahead:5",
						].join("\n"),
						stderr: "",
					};
				}
...
describe("fetchAllTrackedRepos", () => {
	...
	it("collects errors without aborting", async () => {
		...
		expect(result.errors).toEqual([
			{ repo: "github.com/acme/widgets", error: "auth failed" },
		]);
	});

...
describe("resolveLegionPaths", () => {
	it("uses XDG defaults when env vars unset", () => {
		...
		expect(paths.workspacesDir).toBe(
			"/home/testuser/.local/share/legion/workspaces",
		);
		expect(paths.legionsFile).toBe(
			"/home/testuser/.local/state/legion/legions.json",
		);
	});

	it("respects XDG_DATA_HOME", () => {
		const paths = resolveLegionPaths(
	...
	it("computes legion-specific paths", () => {
		...
		expect(legion.legionStateDir).toBe(
			"/home/testuser/.local/state/legion/legions/sjawhar/42",
		);
		expect(legion.logDir).toBe(
			"/home/testuser/.local/state/legion/legions/sjawhar/42/logs",
		);
		expect(legion.workspacesDir).toBe(
			"/home/testuser/.local/share/legion/workspaces/sjawhar/42",
		);
	});
...
describe("path validation", () => {
	it("throws when homeDir is empty string", () => {
		expect(() => resolveLegionPaths({}, "")).toThrow(
			"homeDir must be an absolute path, got: ",
		);
	});
	...
	it("succeeds when repoClonePath is valid", () => {
		...
		expect(result).toBe(
			"/home/test/.local/share/legion/repos/github.com/sjawhar/42",
		);
	});
	...
	it("succeeds when forLegion is valid", () => {
		...
		const legion = paths.forLegion("sjawhar/42");
		expect(legion.legionStateDir).toBe(
			"/home/test/.local/state/legion/legions/sjawhar/42",
		);
	});
});

...
describe("Legion HTTP API", () => {
				...
				if (command.some((part) => part.endsWith("/comments"))) {
					return {
						stdout: JSON.stringify({
							id: 55,
							html_url:
								"https://github.com/acme/widgets/issues/2#issuecomment-55",
						}),
						stderr: "",
						exitCode: 0,
					};
		...
		const deps: LegionApiDeps = {
			...
			processManager: {
				admit: (issue) => {
					...
					return (
						options?.admit?.(issue) ?? options?.admissionResult ?? "spawned"
					);
				},
				releaseSlot: (issue) => {
	...
	it("drains each held event exactly once when a child wave releases", async () => {
		...
		if (
			...
			throw new Error(
				"root start response is missing its architect capability",
			);
		}
	...
	it("requires a daemon-minted single-use boot nonce before root registration", async () => {
		...
		expect(
			(await json("/legion/v1/process/started", input)).response.status,
		).toBe(403);
		const bootToken = await api?.mintBootToken(root, 3);
		if (!bootToken) throw new Error("boot nonce was not minted");
		expect(
	...
	it("emits root catch-up only after the architect has confirmed readiness", async () => {
		...
		const bootToken = await api?.mintBootToken(root, 3);
		if (!bootToken) throw new Error("boot nonce was not minted");
		const started = await json<{ secret: string }>(
	...
	it("writes only inside the caller tree and implements comments, bodies, labels, close, escalation, dispatch registration, gates, admission, backlog, and redacted state", async () => {
		...
		expect(state.issues[child]?.labels).toEqual([
			"legion-child",
			"needs-approval",
		]);
	...
	it("requires the owning architect capability for every architect lifecycle write", async () => {
		...
		if (!rootIssue || !otherTree)
			throw new Error("test state is missing a root issue");
		rootIssue.children.push(child);
		state.issues[child] = {
			key: child,
			title: "Child",
		...
		if (!rootBootToken || !otherBootToken)
			throw new Error("root boot nonces were not minted");
		const rootStarted = await json<{ secret: string }>(
			"/legion/v1/process/started",
			{
				tree: root,
				generation: 3,
		...
		const otherStarted = await json<{ secret: string }>(
			"/legion/v1/process/started",
			{
				tree: otherRoot,
				generation: 1,
		...
		const workerSpawn = await json<{ spawnToken: string }>(
	...
	it("requires a bound architect capability before forwarding Dispatch and registering its returned thread", async () => {
		...
		expect(
			(await json("/legion/v1/dispatch-threads", input)).response.status,
		).toBe(403);
		expect(
			(
		...
		expect(dispatchRequest.headers.get("Authorization")).toBe(
			"Bearer dispatch-bearer",
		);
		expect(state.dispatchThreads).toEqual([
			{
	...
	it("preserves controller authorization and dispatch ownership across a persisted API restart", async () => {
		...
		try {
			...
			if (!competingBootToken)
				throw new Error("competing boot nonce was not minted");
			const competingStarted = await json<{ secret: string }>(
				"/legion/v1/process/started",
				{
					tree: competingRoot,
					generation: 1,
	...
	it("preserves a worker spawn capability across daemon restart so its backed session can register phase", async () => {
		const tempDir = await mkdtemp(
			path.join(os.tmpdir(), "legion-spawn-capability-"),
		);
		const file = path.join(tempDir, "state.json");
		try {
			...
			expect(started.response.status).toBe(200);
			const spawn = await json<{ spawnToken: string }>(
				"/legion/v1/spawn-token",
				{
					tree: root,
					issue: root,
	...
	it("persists a minted controller capability before controller spawn can proceed", async () => {
		const tempDir = await mkdtemp(
			path.join(os.tmpdir(), "legion-controller-capability-"),
		);
		const file = path.join(tempDir, "state.json");
		try {
			await start({ saveState: async () => saveState(file, state) });
	...
	it("rejects a phase whose spawn token was minted for another role", async () => {
		...
		expect(started.response.status).toBe(200);
		const testerSpawn = await json<{ spawnToken: string }>(
			"/legion/v1/spawn-token",
			{
				tree: root,
				issue: root,
		...
		const reviewerSpawn = await json<{ spawnToken: string }>(
			"/legion/v1/spawn-token",
			{
				tree: root,
				issue: root,
				...
				if (command.some((part) => part.endsWith("/comments"))) {
					return {
						stdout: JSON.stringify({
							id: 99,
							html_url:
								"https://github.com/acme/widgets/issues/1#issuecomment-99",
						}),
						stderr: "",
						exitCode: 0,
					};

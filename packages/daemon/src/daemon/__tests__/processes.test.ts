	...
	const deps: ProcessManagerDeps = {
		...
		run: async (command, runnerOptions) => {
			...
			if (
				...
				if (!cloneDir)
					throw new Error("Jujutsu clone is missing its destination");
			...
			if (
				...
				if (!workspaceDir)
					throw new Error("Jujutsu workspace is missing its destination");
...
describe("ProcessManager", () => {
	...
	it("provisions the root issue workspace before launching OMP in that workspace", async () => {
		...
		expect(
			await readFile(path.join(workspace, ".omp", "config.yml"), "utf8"),
		).toBe(
		...
		expect(workspaceCalls).toContainEqual({
			...
			opts: {
				env: {
					GIT_ASKPASS: expect.stringMatching(
						/provisioning-credential-.+\/askpass$/,
					),
	...
	it("caps an escaped cosmetic window name", async () => {
		...
		const window = commands.find(
			(command) => command[0] === "tmux" && command.includes("-n"),
		);
	...
	it("kills and clears a dead tree's tmux locator", async () => {
		const state = newLegionState("omp", 1);
		tree(state);
		if (!state.trees[root]?.locator)
			throw new Error("test root is missing a locator");
	...
	it("passes the packaged extension root to OMP discovery for root and controller windows", async () => {
		...
		expect(
			windows.map((command) => command.includes(`PATH=${panePath}`)),
		).toEqual([true, true]);
	});
		...
		const { manager: processes, commands } = manager(state, {
			...
			run: async (command) => {
				...
				if (command[1] === "new-session")
					return { stdout: "session creation failed", exitCode: 1 };
				return { stdout: "", exitCode: 0 };
			},
		});
	...
	it("promotes the next queued tree when a failed launch releases capacity", async () => {
		...
		const { manager: processes } = manager(state, {
			...
			run: async (command) => {
				if (
					command[1] === "new-window" &&
					command.includes(`LEGION_TREE=${root}`)
				) {
					return { stdout: "root launch failed", exitCode: 1 };
				}
				if (
					command[1] === "new-window" &&
					command.includes(`LEGION_TREE=${child}`)
				) {
					completePromotion?.();
				}
				return { stdout: "", exitCode: 0 };
	...
	it("publishes post-backing events immediately after worker catch-up", async () => {
		...
		const {
			...
			controlRequests,
			publications,
		} = manager(state, { run: liveRun });
		await processes.registerRoleBacking(
			root,
			child,
			"implementer",
			"agent-worker",
		...
		await processes.handleException(
			exception(roleToken("omp", child, "implementer"), original),
		);
	...
	it("spawns one controller window when the controller delivery fails", async () => {
		...
		expect(
			commands.filter((command) => command[1] === "new-window"),
		).toHaveLength(1);
	...
	it("mints a fresh controller capability only for each controller window spawn", async () => {
		...
		expect(windows[0]).toContain(
			"LEGION_CONTROLLER_SECRET=controller-secret-1",
		);
		expect(windows[1]).toContain(
			"LEGION_CONTROLLER_SECRET=controller-secret-2",
		);
	});
	...
	it("persists registered role backing so a restarted manager can revive its durable agent target", async () => {
		...
		await processes.handleException(
			exception(roleToken("omp", child, "architect")),
		);
	...
	it("reclaims a live root architect directly instead of holding its exception", async () => {
		...
		await processes.handleException(
			exception(roleToken("omp", root, "architect")),
		);
	...
	it("persists a dead worker's original delivery before resurrecting its root", async () => {
		...
		expect(recoveredTree.recoveryEvents).toEqual([
			{ issue: child, role, original },
		]);
	});
	...
	it.skipIf(process.env.LEGION_TMUX_LIVE !== "1")(
		...
		async () => {
			...
			const commandRunner = async (command: string[]) => {
				...
				const actual =
					command[1] === "new-window" ||
					(command[1] === "new-session" && command.includes("-n"))
	...
	it.skipIf(process.env.LEGION_TMUX_LIVE !== "1")(
		...
		async () => {
			...
			try {
				...
				if (!firstWindowId)
					throw new Error("live root is missing its tmux window id");
				await commandRunner(["tmux", "kill-pane", "-t", firstWindowId]);
				expect(await processes.probe(root)).toBe("dead");
				await Promise.all([
					processes.resurrect(root),
					processes.resurrect(root),
				]);
				expect(await processes.probe(root)).toBe("alive");
				expect(
					(await commandRunner(["tmux", "list-windows", "-t", session])).stdout,
				).toContain("sjawhar__legion-42");
			} finally {
				await commandRunner(["tmux", "kill-session", "-t", session]);
			}
		},

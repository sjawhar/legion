...
import {
  READY_DELIVERY_ATTEMPTS_PER_CYCLE,
...
type ReadyClaim = { claim: WorkerRoleClaim; locator: Locator } | { stop: string };

interface WorkerReadyContext {
  issue: IssueKey;
  role: LegionRole;
  sessionId: string;
  generation: number;
  root: IssueKey | undefined;
  token: string;
  describe: string;
...
/** One in-flight teardown of a tree's processes (`ProcessManager.closingTrees`): `retire` leaves
 * the record `lingering` (the linger retire), `close` follows the retire with the close.
 * `settled` resolves once the teardown is over, whatever its outcome — a joiner never inherits
 * the starter's rejection. */
interface TreeTeardown {
  kind: "retire" | "close";
...
export class ProcessManager {
  ...
  /** Owns ready-time delivery retry backoff and its cancellable waits. */
  private readonly readyDelivery: ReadyDeliveryRetrier;
  /** Coalesces the extension's duplicate `/worker/ready` retries for one boot generation. */
  private readonly inFlightReadyDeliveries = new Map<string, Promise<void>>();
  /** Bounds and serializes LEGION-102 root-architect delivery re-sends. */
  ...
  /** In-memory only, never persisted: a tree whose processes are being torn down right now — by
   * `retireTreeProcesses` (`kind: "retire"`: the linger retire, which leaves the record
   * `lingering`) or by `closeTree` (`kind: "close"`: the retire followed by the close). A save
   * during the up-to-60s teardown window must never durably record a mid-teardown state a boot
   * cannot resume -- the durable status stays whatever it was (`active`/`lingering`) until the
   * final `closed` save, or (on a stop failure) `lingering` for the periodic sweep to retry. Also
   * makes both entry points idempotent: a second call for the same tree while one is already
   * running awaits the same in-flight teardown instead of racing it (`settled` never rejects; the
   * caller that started the teardown is the one that sees its error), and a `closeTree` that
   * joined a `retire` proceeds to close once it settles (see there). Every `.has` reader keeps
   * one meaning: a teardown of this tree's processes is in flight. */
  ...
  constructor(private readonly deps: ProcessManagerDeps) {
    ...
    this.workerAdmission = new WorkerAdmission({
      ...
      promptExistingWorker: async (client, token, issue, role, sessionId, pending) => {
        if (pending.kind === "assignment") await this.adoptAssignment(issue, role);
        return this.promptExistingWorker(client, token, issue, role, sessionId, pending);
      },
  ...
  /** Cancels every armed boot watchdog, idle-retire clock, ready-delivery backoff sleep, and any
   * pending controller/root registration deadline, and forbids anything from arming again. Called
   * twice by `index.ts`: an in-flight handler during drain (a launch's own success path,
   * `reconnectWorkers`) can still arm a watchdog after the first call, and this is the only
   * guaranteed-safe way to catch that: no background timer may outlive the ProcessManager.
   * Idempotent. */
  dispose(): void {
    this.disposed = true;
   ...
   * `launchWorker`, which spawns a fresh process. A queued prompt carries a stable `deliveryId`
   * from the pending assignment: it is durably saved before its first frame and the worker shim
   * acknowledges a repeated ID without calling OMP again, so a request timeout after the frame
   * reached OMP cannot execute the task twice. Delivery means a started turn, never the shim's
   ...
   * committed nothing; what "the prompt failed" means is the caller's: `promoteQueuedWorker`
   * counts a refusal at once, `workerReady` retries it on the ready-delivery schedule
   * (LEGION-39) and counts only an exhausted cycle. Does not touch the running-worker admission
   * count itself — the caller owns reserving and releasing that slot (mirroring `launchWorker`,
   * which is likewise unaware of admission bookkeeping) since only the caller knows whether this
   * prompt represents a new admission (`resumeOrQueueExisting`'s below-cap idle-resume, or a
   * queue promotion) or none at all (`/worker/ready` resuming a worker whose slot was already
   * counted via its locator from the moment `launchWorker` wrote it, so nothing here releases
   * capacity). The caller adopts an assignment before prompting, so a failure leaves the worker
   * unprompted and the claim untouched. Ready delivery adopts the pending assignment selected
  ...
  private async promptExistingWorker(
    ...
    const receipt = await client.prompt(pending.task, pending.deliveryId, () => {
      void this.requeueLateRefusedPrompt(token, issue, role, sessionId, pending).catch((error) => {
        console.error(`[legion] failed to requeue late-refused prompt for ${token}:`, error);
      });
    });
  ...
  /** A second OMP refusal can follow its immediate prompt acknowledgement after a foreign turn
   * starts during `AgentSession.prompt`'s pre-streaming cleanup. Undo only this prompt's still
   * active phase; a completion or a newer assignment wins the race and needs no mutation. */
  private async requeueLateRefusedPrompt(
    token: string,
    issue: IssueKey,
    role: LegionRole,
    sessionId: string,
    ...
    if (treeKey === undefined) return;
    await this.workerAdmission.mutateClaim(token, async () => {
      const claim = this.deps.state.roles[token];
      const phase = this.deps.state.phases[issue];
      if (
        !claim ||
        !("issue" in claim) ||
      ...
      if (pending.kind === "assignment") {
        if (
          !phase ||
          phase.completed !== undefined ||
          phase.phase !== role ||
          phase.sessionId !== sessionId
        ) {
          return;
        }
        delete this.deps.state.phases[issue];
      }
      console.error(`[legion] ${token}: prompt was refused after its acknowledgement; re-queuing`);
      claim.pendingAssignment = pending;
  ...
  /**
   * Delivers a phase worker's queued assignment (or, with nothing queued, confirms its boot) once
   * it has posted `/worker/ready`: connects its shim socket, prompts `pendingAssignment`, and
   * records `readyConfirmedAt`. Under host load that connect or first prompt can time out
   * (`worker_rpc_timeout_seconds`), and nothing else ever redelivers a queued task to a worker
   * whose boot is unconfirmed, so this method retries the whole delivery itself on the
   * `READY_DELIVERY_RETRY_DELAYS_MS` schedule (LEGION-39): every attempt re-enters the role's
   * `mutateClaim` critical section, re-validates the claim — same session id and generation,
   * locator still present, boot still unconfirmed, tree not closing — and delivers whatever
   * `pendingAssignment` is queued *at that moment* (latest wins: while a boot is unconfirmed,
   * `spawn_worker` replaces the queued task and relies on this very loop, since the extension posts
   * `/worker/ready` exactly once). Nothing is held across a sleep. The loop retries the failures
   * it was built for — a connect refused or timed out, `negotiate_protocol` timed out, the prompt
   * request itself failing or timing out — and ends when the prompt lands (the worker's turn is
   * observed to start, LEGION-60), when the claim has moved on (a stop line names the reason on
   * every attempt but the loop's first), when the daemon is disposed, or with a rethrown
   * `TreeClosingError`/`StopFailed`. A refused `{success:false}` prompt is retried on this
   * schedule and charges only when a cycle exhausts. A prompt the worker ACKNOWLEDGED without
   * starting a turn (`PromptNotStarted`) is terminal for this loop: a `no-turn` confirms the
   * boot, queues the task for promotion, and counts one `promptFailures`
   * (`queueUnstartedPrompt` — LEGION-60's own accounting; a later cycle can therefore add its
   * own exhausted-cycle charge); a `socket-closed` confirms, counts, and queues nothing and
   * stops the loop for the socket-close handler (`onWorkerClientClosed` ->
   * `retireUnconfirmedBoot`) to retire the unconfirmed boot — retrying would race that handler
   * inside the same role lock. A cycle that ends undelivered on the retried failures counts one
   * `promptFailures` through `WorkerAdmission.recordPromptFailure` — never `launchFailures`, and
   * never per attempt — and the next cycle follows after the schedule's last delay; at
   * `MAX_LAUNCH_FAILURES` the breaker retires the pane, queues the token, and the still-queued
   * task relaunches cold with `--resume`. One ready delivery is therefore charged by exactly one
   * of the two paths at a time.
   * A breaker retire whose stop fails (`StopFailed`) ends nothing: the failure is logged and the
   * loop continues, since `retireWorkerLocator` has already cancelled the boot watchdog by then and
   * this loop is the only thing left watching the claim — the next exhausted cycle retries the
   * retire until it lands or the claim moves on. While the loop runs the boot stays unconfirmed, so
   * the boot watchdog keeps judging the pane and its registration deadline retires an
   * alive-but-undelivered one through its own `launchFailures` path, which bumps the generation and
   * so ends this loop at its next attempt — under defaults that deadline typically lands before a
   * cycle of timed-out attempts can exhaust (six timeouts plus 335 s of sleeps against ~360 s from
   * launch), not necessarily before a cycle of instantly-refused ones.
   *
   * Settles only when the delivery ends, so a request path must never await it: `handleWorkerReady`
   * detaches it and logs whatever escapes.
   */
  workerReady(
    issue: IssueKey,
    role: LegionRole,
    ...
    const token = roleToken(this.deps.state.project, issue, role);
    const key = `${token}:${generation}`;
    const existing = this.inFlightReadyDeliveries.get(key);
    if (existing) return existing;
    const delivery = this.deliverWorkerReady(issue, role, sessionId, generation).finally(() => {
      this.inFlightReadyDeliveries.delete(key);
    });
    this.inFlightReadyDeliveries.set(key, delivery);
    return delivery;
  }

  private async deliverWorkerReady(
    issue: IssueKey,
    role: LegionRole,
    sessionId: string,
    generation: number
    ...
    if (root && this.closingTrees.has(root)) throw new TreeClosingError(root);
    const token = roleToken(this.deps.state.project, issue, role);
    const context: WorkerReadyContext = {
      issue,
      role,
      sessionId,
      generation,
      root,
      token,
      describe: `worker/ready delivery for ${issue}/${role}`,
    };
    await this.readyDelivery.run(
      context.describe,
      (attemptNumber, cycle) => this.deliverReadyAttempt(context, attemptNumber, cycle),
      {
        onFirstFailure: (error) =>
          console.error(
            `[legion] failed to deliver worker/ready assignment for ${context.issue}/${context.role}:`,
            error
          ),
        onCycleExhausted: (cycle, lastError) =>
          this.chargeExhaustedReadyCycle(context, cycle, lastError),
      }
    );
  }

  private readyClaim(context: WorkerReadyContext): ReadyClaim {
    if (context.root && this.closingTrees.has(context.root)) {
      throw new TreeClosingError(context.root);
    }
    const claim = this.deps.state.roles[context.token];
    if (!claim || !("issue" in claim)) return { stop: "the claim is gone" };
    if (claim.sessionId !== context.sessionId || claim.generation !== context.generation) {
      return {
        stop: `superseded — the claim is now generation ${claim.generation} for session ${claim.sessionId}`,
      };
    }
    if (!claim.locator) return { stop: "its locator was cleared" };
    if (claim.readyConfirmedAt !== undefined) {
      this.cancelBootWatchdog(context.token, context.generation);
      return { stop: "the boot was already confirmed" };
    }
    return { claim, locator: claim.locator };
  }

  private stopReadyRetries(context: WorkerReadyContext, reason: string): void {
    console.info(
      `[legion] stopping worker/ready delivery retries for ${context.issue}/${context.role}: ${reason}`
    );
  }

  private async deliverReadyAttempt(
    context: WorkerReadyContext,
    attemptNumber: number,
    cycle: number
  ): Promise<void> {
    let retiredBystander = false;
    await this.workerAdmission.mutateClaim(context.token, async () => {
      const live = this.readyClaim(context);
      if ("stop" in live) {
        if (cycle > 1 || attemptNumber > 1) this.stopReadyRetries(context, live.stop);
        return;
      }
      const { claim, locator } = live;
      const pending = claim.pendingAssignment;
      if (pending?.kind === "assignment" && context.adoptedDeliveryId !== pending.deliveryId) {
        await this.adoptAssignment(context.issue, context.role);
        context.adoptedDeliveryId = pending.deliveryId;
      }
      // This fresh RPC pane has nothing it may be prompted with; confirming it would strand a
      // worker-cap slot until a later timeout, so the bystander path retires it immediately.
      if (isBystanderCatchup(this.deps.state, context.issue, context.role, pending)) {
        console.info(
          `[legion] dropping queued catch-up for ${context.token} at ready and retiring the relaunched pane: ${context.issue}'s active phase is ${activePhaseLabel(this.deps.state, context.issue)}; only spawn_worker resumes a finished worker`
        );
        delete claim.pendingAssignment;
        // Confirm before stopping: `retireWorkerLocator` cancels the watchdog before a
        // `StopFailed`, and a confirmed locator keeps the idle-retirement recovery shape intact.
        claim.readyConfirmedAt = this.deps.now();
        delete claim.launchFailures;
        await this.retireWorkerLocator(context.token, locator);
      ...
      try {
        client = await this.clientFor(context.token, locator);
      } catch (error) {
        throw new ReadyDeliveryTransportError(error);
      }
      ...
      if (pending) {
        try {
          ...
          if (!(error instanceof PromptNotStarted)) throw new ReadyDeliveryTransportError(error);
          if (context.root === undefined) throw error;
          if (error.reason === "socket-closed") {
            console.error(
              `[legion] ${context.token}: ${error.message}; boot left unconfirmed for the socket-close handler to retire`
            );
            this.stopReadyRetries(
              context,
              "the worker's socket closed before its turn started; the socket-close handler owns the claim"
            );
            return;
          }
          confirmBoot();
          this.cancelBootWatchdog(context.token, context.generation);
          console.error(
            `[legion] ${context.token}: ${error.message}; boot confirmed, task queued for promotion`
          );
          this.stopReadyRetries(
            context,
            "the worker acknowledged the prompt but started no turn; boot confirmed, task queued for promotion"
          );
          await this.workerAdmission.queueUnstartedPrompt(
            context.token,
            context.issue,
  ...
  private async chargeExhaustedReadyCycle(
    context: WorkerReadyContext,
    cycle: number,
    lastError: ReadyDeliveryTransportError
  ): Promise<"stop" | "continue"> {
    let breakerRetired = false;
    try {
      return await this.workerAdmission.mutateClaim(
        context.token,
        async (): Promise<"stop" | "continue"> => {
          const live = this.readyClaim(context);
          if ("stop" in live) {
            this.stopReadyRetries(context, live.stop);
            return "stop";
          }
          let promptFailure: { failures: number; retired: boolean; died: boolean };
          try {
            promptFailure = await this.workerAdmission.recordPromptFailure(context.token);
          } catch (error) {
            console.error(
              `[legion] ${context.describe}: could not retire ${context.issue}/${context.role}'s pane at the breaker; continuing ready-delivery cycles: ${error instanceof Error ? error.message : String(error)}`
            );
            await this.persist();
            return "continue";
          }
          console.error(
            `[legion] ${context.describe} exhausted cycle ${cycle} (${READY_DELIVERY_ATTEMPTS_PER_CYCLE} attempts; last: ${lastError.message}); prompt failure ${promptFailure.failures}/${MAX_LAUNCH_FAILURES}`
          );
          breakerRetired = promptFailure.retired;
          if (breakerRetired && !promptFailure.died) {
            console.error(
              `[legion] retiring ${context.issue}/${context.role}'s pane after ${MAX_LAUNCH_FAILURES} exhausted ready-delivery cycles; its queued task relaunches cold with --resume`
            );
          }
          await this.persist();
          return breakerRetired ? "stop" : "continue";
        }
      );
    } finally {
  ...
  private async retireUnconfirmedBoot(
    ...
    await this.workerAdmission.mutateClaim(token, async () => {
      ...
      // generation the teardown never asked for. A `lingering` tree is the same case a tick
  ...
  /**
   * Stops every process a tree's root and worker claims record and leaves the tree record itself
   * where it is — the linger retire (LEGION-105): `beginLinger` starts it in the background,
   * `reportRootExit`'s not-closing branch starts it for the workers a self-exited root leaves
   * behind, and `retireLingeringTrees` (the sweep tick and the boot pass) runs it for every
   * lingering tree that still records a locator or a claim. The body is
   * `retireTreeProcessesLocked`, the same primitive `closeTreeLocked` runs before the close, so a
   * linger stops a tree exactly as a close does. Idempotent through `closingTrees`: a call that
   * finds any teardown of the tree in flight — a retire or a close — awaits it and returns without
   * stopping anything twice. Rejects with `StopFailed` when a process would not stop (that
   * process's locator and claim are kept; see the primitive); a joiner never inherits that
   * rejection, the starter alone reports it. One log line names what a retire that found
   * something to retire did (a close stays as quiet as before).
   */
  async retireTreeProcesses(treeKey: IssueKey): Promise<void> {
    ...
    const retiring = this.retireTreeProcessesLocked(treeKey).finally(() => {
      this.closingTrees.delete(treeKey);
    });
    this.closingTrees.set(treeKey, {
      kind: "retire",
      settled: retiring.then(
        () => {},
        () => {}
      ),
    });
    const retired = await retiring;
    if (retired.root !== "none" || retired.stoppedWorkers > 0 || retired.removedClaims > 0) {
      console.error(
  ...
  /**
   * Closes a tree: marks it `lingering` (its first durable act, if it is not already), retires
   * every process it records (`retireTreeProcessesLocked` — root and every worker concurrently,
   * each asked to shut down over its own shim socket and given up to the configured tree stop
   * timeout before `stopProcess` has the runtime kill it directly), removes the workspaces of its
   * finished issues, and turns the record `closed`. Idempotent through `closingTrees` (in-memory
   * only — see its field comment for why this is never persisted): a call that finds a `close` in
   * flight awaits it and returns; one that finds a `retire` in flight — the linger retire
   * `beginLinger` started moments ago, the common shape when `expireLinger` follows a linger —
   * awaits it and then closes, so a linger followed by its expiry still ends `closed`. Marking the
   * tree `lingering` with a fresh `lingerUntil` before any stop means a crash anywhere during the
   * close leaves a retryable `lingering` tree the periodic sweep re-closes, never a durably
   * `active` record with an already-half-stopped process underneath it. A process whose stop
   * fails (`StopFailed` — a real runtime stop failure, not the routine "process already gone"
   * case) keeps its claim and locator: the tree is left `lingering` with a fresh `lingerUntil` so
   * the periodic sweep retries the close, and this call throws `StopFailed` rather than reporting
   * a false success. The admission slot a mid-close `admit` took is released by `closeTreeLocked`
   * but handed on only here, once `closingTrees` no longer names the tree: the promotion sweep
   * launches roots (`advancePromotionSweep` → `startRoot`), and a launched root first awaits any
   * in-flight teardown of its own tree or an ancestor's (`awaitClosingTrees`) — so a queued child
   * of this tree, promoted from inside the frame `closingTrees` still named, would wait on the
   * very close that was promoting it and neither would ever settle (found in LEGION-104's
   * review). Nothing the fence protects needs it any longer by then: every claim is deleted and
  ...
  private async closeTreeLocked(treeKey: IssueKey): Promise<boolean> {
    ...
    try {
      ...
      // A process that would not stop keeps the tree lingering with an already-expired
      // `lingerUntil`, so the next sweep tick retries the close (the surviving locator and claim
      // are exactly where the primitive left them). Any other failure — the primitive's own
      // persist rejecting — propagates untouched, exactly like a rejected save anywhere else.
    ...
    // The reopen window ends with the record: a re-admitted closed issue starts a fresh architect
    // (the linger retire kept this path only for a `todo` inside the window).
  ...
  /**
   * The one teardown of a tree's processes, shared by the linger retire (`retireTreeProcesses`)
   * and the close (`closeTreeLocked`); never writes `tree.status` or `tree.lingerUntil`. The root
   * leg stops the root only if `probe` confirms it is still alive — a dead root has nothing to
   * gracefully close, so `stopProcess` skips straight to reaping whatever process is left rather
   * than burning the full stop timeout finding that out again — moves the root's `ompSessionFile`
   * into `tree.resumeSessionFile` (a `todo` re-admission inside the linger window resumes the same
   * architect; `closeTreeLocked` drops it again) and clears the locator. The worker leg runs to a
   * fixed point: `inFlightLaunches` (see its field comment) is awaited before every re-snapshot of
   * `state.roles`, so a `spawnWorker` call that crossed the closing check just before
   * `closingTrees` was populated always gets to finish (and be seen) before a snapshot can report
   * the tree empty — every claim present at any point during this call gets exactly one stop
   * attempt. Then every claim under the tree that had no locator to stop (the architect's, a
   * queued never-launched worker's, a retired idle worker's) is deleted and revoked, the tree's
   * phases are cleared, its queue entries pruned, its spawn capabilities deleted, and the state
   * persisted. A process whose stop fails (`StopFailed` — a real runtime stop failure, not the
   * routine "process already gone" case) is never deleted and its locator never cleared — a
   * possibly-still-live process must never lose its only durable handle — and this call throws
   * `StopFailed` after the persist so the caller retries it on the next sweep tick. Resolves to
  ...
  private async retireTreeProcessesLocked(treeKey: IssueKey): Promise<{
    ...
    if (tree.locator) {
      ...
      // Before the stop is even attempted, never after it resolves: a stop that cannot be
      // confirmed keeps the locator for the sweep's retry, and the durable record must carry the
      // transcript by then — a `todo` re-admission inside the window derives its `--resume` from
      // `tree.resumeSessionFile`, so setting this only on success let a failed root leg reopen as
      // a fresh agent over a process the daemon could not prove stopped (LEGION-105 deep review).
      if (rootLocator.ompSessionFile !== undefined) {
        tree.resumeSessionFile = rootLocator.ompSessionFile;
      }
    ...
    for (;;) {
      ...
      for (const [index, [token]] of batch.entries()) {
        ...
        if (result?.status === "fulfilled") {
          ...
          anyFailed = true;
          console.error(
            `[legion] worker ${token} failed to stop while retiring the processes of ${treeKey}:`,
    ...
    this.clearTreePhases(treeKey);
    // A tree torn down while it still had queued (never-launched) tokens must never leave them
    // behind for a later, unrelated drain to promote against a tree that no longer runs.
    await this.workerAdmission.pruneQueueForTree(treeKey);
    // A spawn capability minted before the teardown has no live claim left to authorize once this
    ...
    // capability recorded against this tree closes that window for good (a `todo` re-admission
    // mints a fresh one in `spawnTree`).
    ...
    return { root: rootOutcome, stoppedWorkers, removedClaims };
  }
  ...
  /** Awaits every in-flight teardown (`closeTree`, or the linger retire `retireTreeProcesses`)
   * for `issue` or an ancestor of it before a root launch provisions its workspace: a close may
   * be removing exactly this directory (`removeTreeWorkspaces`), and a retire is still stopping
   * the processes the relaunch replaces. A root re-admitted while its previous tree still closes
   * (a human moves a finished issue back to `todo` within the close's stop window) and a child
   * re-admitted as its own root while its parent's tree closes both wait here; the close itself
   * removes nothing for a record that is no longer `lingering`. A failed teardown is its own
   * caller's to log — the launch proceeds either way, exactly as it does today. Phase-worker
   * launches need no wait: `launchWorker` refuses a closing tree outright (`TreeClosingError`). */
  private async awaitClosingTrees(issue: IssueKey): Promise<void> {
    ...
    for (
      ...
      seen.add(current);
      const teardown = this.closingTrees.get(current);
      if (!teardown) continue;
      console.error(
        `[legion] launch of ${issue} waits for the ${teardown.kind} of tree ${current} to finish`
      );
      await teardown.settled;
    }
  }
  ...
  async spawnRoot(issue: IssueKey, resume = false, resumeSessionFile?: string): Promise<void> {
    ...
    try {
      // A tree that still records a locator here is one whose teardown could not confirm its root
      // stopped (`retireTreeProcessesLocked`'s `StopFailed` path keeps the locator, and the
      // rollback below restores one for a failed launch): every other path clears it before
      // launching (`resurrectDeadTree` through `removeTreeProcess`, the retire on success).
      // Opening a pane over it would overwrite the only handle to a process that may still be
      // alive and adopt its transcript under a fresh agent, so the retained process is stopped
      // first and a stop that still cannot be confirmed refuses this launch — `spawnRoot`'s own
      // rollback then requeues the tree with its session file for the promotion sweep (or the
      // next `todo`) to retry, and escalates to `launch-failed` at the bound like any other
      // launch that cannot happen (LEGION-105 deep review).
      if (await this.stopRetainedRootProcess(tree)) priorLocator = undefined;
      // A tree that kept its session file across a cleared pane (`recordRootExit`, the linger
      // retire — including the failed root leg, which records it before the stop — and the at-cap
      // branch of `resurrectDeadTree`) is always resumed, whatever the caller asked: `admit` and
      // the promotion sweep ask for a fresh launch because they cannot tell a never-started tree
      // from a requeued one (LEGION-83). Whether the launch is an admission is a separate question
      // from what it resumes — a `todo` re-admission of a lingering tree resumes the kept
      // transcript AND is admitted (`admittedIssues`), so it writes `in_progress` like a fresh
      // admission; a resurrection resumes and writes nothing.
  ...
  private async connectOnReady(
    ...
    await this.readyDelivery.run(
      describe,
      async (attemptNumber) => {
        const stop = stillCurrent();
        if (stop !== undefined) {
          if (attemptNumber > 1) console.info(`[legion] stopping ${describe} retries: ${stop}`);
          return;
        }
        try {
          await this.clientFor(token, locator);
        } catch (error) {
          throw new ReadyDeliveryTransportError(error);
        }
      },
      {
        onFirstFailure: (error) => console.error(firstFailure, error),
        onCycleExhausted: async (_cycle, lastError) => {
          console.error(
            `[legion] giving up on ${describe} after ${READY_DELIVERY_ATTEMPTS_PER_CYCLE} connect attempts: ${lastError.message}`
          );
          return "stop";
        },
      }
  ...
  /** Connects the root architect's shim socket on `/process/ready`, exactly as `workerReady`
   ...
   * method. Best-effort, and never rejects for a connect failure: a connect that times out under
   * host load is retried for exactly one `READY_DELIVERY_RETRY_DELAYS_MS` cycle (LEGION-39), each
   * attempt re-checking that this tree is still the same active generation on the same process
   * and that the daemon is still running; exhaustion logs and stops — nothing is retired, no
   * counter moves — and the next `spawnWorker`/reconnect/probe that touches the socket reconnects.
   * `requireTree`'s throw for an unknown tree stays ahead of the loop. Settles only when the cycle
   * ends, so `handleProcessReady` detaches it. */
  async markTreeReady(treeKey: IssueKey): Promise<void> {
    const tree = this.requireTree(treeKey);
    const startLocator = tree.locator;
    if (!startLocator) return;
    const startGeneration = tree.generation;
    const token = roleToken(this.deps.state.project, treeKey, "architect");
    await this.connectOnReady(
      `architect shim connect for ${treeKey}`,
      `[legion] failed to connect architect shim socket on ready for ${treeKey}:`,
      token,
      startLocator,
      () => {
        const current = this.deps.state.trees[treeKey];
        if (!current) return "the tree is gone";
        if (current.status !== "active") return `the tree is ${current.status}`;
        if (current.generation !== startGeneration) {
          return `the tree moved to generation ${current.generation}`;
        }
        if (!sameProcess(current.locator, startLocator)) return "the tree's process was replaced";
        return undefined;
      }
    );
  }
  ...
  /**
   * `spawnRoot`'s fence: stops the process a retained `tree.locator` still names before anything
   * is launched for the tree, and answers whether it cleared that locator. Returns `false`
   * immediately for a tree with no locator — every ordinary launch (a first admission, a
   * `todo` re-admission of a retired lingering tree, a resurrection past `removeTreeProcess`)
   * takes that path and pays nothing. A locator survives to here only from a teardown whose stop
   * could not be confirmed (`retireTreeProcessesLocked`'s `StopFailed`) or a rolled-back launch,
   * which is exactly the process that must not be orphaned under a fresh pane. The transcript is
   * recorded on the tree first (the same reason the retire records it before its own stop), then
   * the process is asked to exit over its own role-scoped socket with the same probe-derived
   * options the retire uses, under `stoppingForRelaunch` so the root's own `/process/exit` during
   * that stop is not read as a death. A `StopFailed` — or a probe the runtime could not complete
   * — propagates: the caller's rollback requeues the tree with its session file rather than
   * opening a second root (LEGION-105 deep review). The claim is left alone: it carries the
   * session the resumed launch must expect, and the teardown that finally stops this process
   * deletes both together.
   */
  private async stopRetainedRootProcess(tree: TreeState): Promise<boolean> {
    const locator = tree.locator;
    if (!locator) return false;
    const architectToken = roleToken(this.deps.state.project, tree.root, "architect");
    if (locator.ompSessionFile !== undefined) tree.resumeSessionFile = locator.ompSessionFile;
    const verdict = await this.probeTree(tree.root);
    console.error(
      `[legion] stopping ${tree.root}'s retained root process before its relaunch: a teardown could not confirm it stopped (${verdict.status === "alive" ? "alive" : `dead/${verdict.reason}`})`
    );
    this.stoppingForRelaunch.add(tree.root);
    try {
      await this.stopProcessSerialized(architectToken, locator, this.treeStopTimeoutMs, {
        skipGraceful: verdict.status === "dead" && verdict.reason === "gone",
        refuseKill: verdict.status === "dead" && verdict.reason === "not-recorded-process",
      });
    } finally {

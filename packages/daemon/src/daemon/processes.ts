...
export class ProcessManager {
	...
	async registerRoleBacking(
		...
		if (this.rootForIssue(issue) !== treeKey) {
			throw new Error(
				`Issue ${issue} does not belong to Legion tree ${treeKey}`,
			);
		}
	...
	async reconcileTmuxWindows(): Promise<void> {
		...
		const known = new Set(
			[
				...Object.values(this.deps.state.trees).map(
					(tree) => tree.locator?.tmuxWindowId,
				),
	...
	async spawnRoot(
		...
		try {
			...
			if (activeIndex !== -1)
				this.deps.state.admission.active.splice(activeIndex, 1);
			if (tree.launchFailures >= MAX_LAUNCH_FAILURES) {
				...
				if (queueIndex !== -1)
					this.deps.state.admission.queue.splice(queueIndex, 1);
	...
	async controlDirective(
		...
		if (generation === undefined)
			throw new Error(`Unknown Legion tree: ${tree}`);
	...
	async markTreeReady(treeKey: IssueKey): Promise<void> {
		...
		for (const recovery of [...recoveries]) {
			...
			if (recovery.issue === treeKey && recovery.role === "architect") {
				this.deps.natsPublish(
					recovery.original.topic,
					recovery.original.payload,
				);
				recovered = true;
			} else {
	...
	async expireLinger(treeKey: IssueKey): Promise<void> {
		void this.controlDirective(treeKey, { type: "shutdown" }).catch((error) => {
			console.error(
				`[legion] shutdown directive failed for ${treeKey}:`,
				error,
			);
		});
	...
	private async reviveWorker(
		...
		if (!sessionFile)
			throw new Error(`Live Legion tree ${root} has no OMP session file`);
		...
		const catchup = await workerCatchup(
			this.deps.state,
			issue,
			role,
			this.deps.workerCatchup,
		);
		this.deps.natsPublish(
	...
	private advancePromotionSweep(): void {
		...
		if (!sweep) return;
		if (
			this.deps.state.admission.active.length >= this.deps.state.admission.cap
		) {
	...
	private async spawnTree(
		...
		const workspace = await provisionIssueWorkspace(tree.root, {
			...
			provisioningToken: async () =>
				await this.deps.provisioningToken(parsedTree.owner),
	...
	private async spawnController(controllerSecret: string): Promise<void> {
		...
		await (this.deps.statPrompt ?? stat)(promptPath);
		await this.writeOmpConfig(
			controllerDir,
			this.deps.config.maxRecursionDepth,
		);
		const session = `legion-${this.deps.state.project}`;
	...
	private async openTmuxWindow(
		...
		const sessionExists =
			(await this.deps.run(["tmux", "has-session", "-t", session])).exitCode ===
			0;
		...
		if (result.exitCode !== 0) {
			throw new Error(
				`tmux ${command[1]} failed (exit ${result.exitCode}): ${result.stdout}`,
			);
		}
		if (!sessionExists) {
			...
			if (marker.exitCode !== 0) {
				throw new Error(
					`tmux ownership marker failed (exit ${marker.exitCode}): ${marker.stdout}`,
				);
			}
		}
		...
		if (!/^@\d+$/.test(tmuxWindowId)) {
			throw new Error(
				`tmux ${command[1]} did not report a window id: ${result.stdout}`,
			);
		}
		return tmuxWindowId;
	}
	...
	private async isOmpPane(pid: number): Promise<boolean> {
		if ((await this.deps.run(["kill", "-0", String(pid)])).exitCode !== 0)
			return false;
		try {
	...
	private redeliverHeldRoleEvents(
		treeKey: IssueKey,
		issue: IssueKey,
		role: LegionRole,
	): void {

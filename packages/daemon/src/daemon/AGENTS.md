...
| `POST /legion/v1/issues/status`, `/waves/release`, `/gates/register` | Scoped architect writes (any status in its tree; `release_wave` PATCHes `todo` for every key in `issues`; gate registers a design-ask id — and, when `config.gates.design` is `off`, records `designApproved: "gate-off"` in the same write and publishes `{type:"design-approved"}` to the tree's architect role itself, best-effort, since the state already carries the approval for catch-up; boot applies the same marker to every gate registered but never approved, so turning the gate off unblocks trees parked under it) — and the controller capability may additionally set `todo`\|`backlog`\|`icebox` on any project issue via `/issues/status`. |
...
- `DaemonConfig` supplies all lifecycle defaults: admission cap, worker cap, recursion depth, linger duration, resync interval, retry limit, the two stop timeouts (`workerStopTimeoutSeconds`, default 10; `treeStopTimeoutSeconds`, default 60), worker boot timeout (`worker_boot_timeout_seconds`/`LEGION_WORKER_BOOT_TIMEOUT_SECONDS`, default 120), and worker RPC timeout (`worker_rpc_timeout_seconds`/`LEGION_WORKER_RPC_TIMEOUT_SECONDS`, default 5).

...
function configuredPath(
	...
	if (!path.isAbsolute(configured)) {
		throw new Error(
			`LEGION_${tool.toUpperCase()}_PATH must be an absolute executable path`,
		);
	}
	return configured;
}
...
function resolveConfiguredOrFound(
	...
	return resolveExecutable(
		configured ?? tool,
		configured ? undefined : searchPath,
	);
}
...
function parseMiseEnvironment(stdout: string): FullMiseEnvironment {
	...
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
		throw new Error(
			"[legion] mise env --json did not return an environment object",
		);
	}
	...
	if (entries.some(([, value]) => typeof value !== "string")) {
		throw new Error(
			"[legion] mise env --json returned a non-string environment value",
		);
	}
...
async function resolveOmpInvocation(
	...
	if (configured) {
		...
		throw new Error(
			`[legion] LEGION_OMP_PATH is not an executable: ${configured}`,
		);
	}
	...
	const resolved =
		result.exitCode === 0
			? resolveExecutable(path.join(installDir, "bin", "omp"))
			: undefined;
	if (resolved) return resolved;
...
export async function resolveDaemonEnvironment(
	...
	const mise = resolveConfiguredOrFound(
		"mise",
		env,
		env.PATH,
		resolveExecutable,
	...
	for (const tool of REQUIRED_DAEMON_TOOLS) {
		const resolved = resolveConfiguredOrFound(
			tool,
			env,
			paneEnv.PATH,
			resolveExecutable,
	...
	if (missing.length > 0) {
		throw new Error(
			`[legion] Missing required daemon tools: ${missing
				.map(
					(tool) =>
						`${tool} (set LEGION_${tool.toUpperCase()}_PATH to an absolute executable path)`,
				)
				.join(", ")}`,
		);
	}

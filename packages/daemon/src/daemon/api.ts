...
function roleForSession(
	...
	const declaredPhaseRole = LEGION_ROLES.find(
...
export function startLegionApi(
	...
	const tokenForIssue = async (
		issue: IssueKey,
		appRole: GitHubAppRole,
	): Promise<TokenLease> => {
		const parsed = parseIssueKey(issue);
		if (!parsed) {
			throw new Error(`Invalid issue key in Legion state: ${issue}`);
	...
	const gh = async (
		...
		if (result.exitCode !== 0) {
			throw new Error(
				`GitHub command failed: ${result.stderr || result.stdout}`,
			);
		}
	...
	const dispatchThread = async (
		...
		if (!response.ok) {
			throw new Error(
				`Dispatch MCP request failed with ${response.status}: ${responseBody}`,
			);
		}
		...
		if (
			...
			throw new Error(
				"Dispatch MCP result must contain an integer thread and URL",
			);
		}
	...
	const appendFooter = (
		...
		const session =
			deps.state.roles[roleToken(deps.state.project, tree, "architect")]
				?.sessionId ?? "";
	...
	const requireArchitectCapability = (
		...
		const capability = requireSessionCapability(
			body,
			tree,
			issuedCapability?.issue ?? tree,
		);
		if (capability.role !== "architect") {
			throw new HttpError(
				403,
				"Only an architect may perform lifecycle writes",
			);
		}
		return capability;
	};
		...
		try {
			...
			if (request.method === "GET" && pathname === "/legion/v1/state") {
				...
				return Response.json(
					validateContractResponse(LegionDaemonApi.State.response, redacted),
				);
			}
			...
			if (pathname === "/legion/v1/process/ready") {
				...
				return Response.json(
					validateContractResponse(LegionDaemonApi.ProcessReady.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/process/exit") {
				...
				return Response.json(
					validateContractResponse(LegionDaemonApi.ProcessExit.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/issues") {
				...
				if (labels.some((label) => label !== "needs-approval")) {
					throw new HttpError(
						400,
						"Architect issue creation only accepts needs-approval",
					);
				}
				...
				const parent = asRecord(
					JSON.parse(await gh(tree, ["gh", "api", issueUrl(tree)])),
				);
			...
			if (pathname === "/legion/v1/issues/comment") {
				const { tree, issue } = requireTreeIssue(body);
				requireArchitectCapability(body, tree);
				const commentBody = appendFooter(
					tree,
					issue,
			...
			if (pathname === "/legion/v1/issues/body") {
				...
				return Response.json(
					validateContractResponse(LegionDaemonApi.PostBody.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/issues/labels") {
				...
				if (
					add.some((label) => label !== "needs-approval") ||
					remove.length > 0
				) {
					throw new HttpError(
						400,
						"Architect label changes only add needs-approval",
					);
				}
				if (add.length > 0) {
					await gh(issue, [
			...
			if (pathname === "/legion/v1/issues/close") {
				...
				await save();
				return Response.json(
					validateContractResponse(LegionDaemonApi.IssueClose.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/escalate") {
				...
				return Response.json(
					validateContractResponse(LegionDaemonApi.Escalate.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/dispatch-threads") {
				...
				if (role !== capability.role) {
					throw new HttpError(
						403,
						"Dispatch role does not match the authenticated session",
					);
				}
				if (role !== "architect") {
					throw new HttpError(
						403,
						"Only an architect may open a Dispatch thread",
					);
				}
				...
				if (!treeContains(deps.state, tree, parent)) {
					throw new HttpError(
						403,
						"Dispatch parent is outside the caller tree",
					);
				}
				...
				const dispatched = await dispatchThread(
					parent,
					subject,
					dispatchBody,
					body.ask,
				...
				const repo =
					`${parsedIssue.owner}/${parsedIssue.repo}` as `${string}/${string}`;
				const existingMapping = deps.state.dispatchThreads.find(
					(entry) => entry.repo === repo && entry.thread === dispatched.thread,
				);
				if (
					existingMapping &&
					(existingMapping.tree !== tree || existingMapping.issue !== issue)
				) {
					throw new HttpError(
						403,
						"Dispatch thread is already owned by another issue",
					);
				}
				...
				return Response.json(
					validateContractResponse(
						LegionDaemonApi.DispatchThread.response,
						dispatched,
					),
				);
			}
			...
			if (pathname === "/legion/v1/role-backing") {
				...
				await deps.processManager.registerRoleBacking(
					tree,
					issue,
					role,
					agentId,
				...
				await save();
				return Response.json(
					validateContractResponse(LegionDaemonApi.RoleBacking.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/provisioning-credential") {
				...
				validateContractRequest(
					LegionDaemonApi.ProvisioningCredential.request,
					body,
				);
				return Response.json(
					validateContractResponse(
			...
			if (pathname === "/legion/v1/git-credential") {
				...
				const lease = await tokenForIssue(
					grant.issue,
					appRoleForLegionRole(grant.role),
				);
				return new Response(
			...
			if (pathname === "/legion/v1/controller/ready") {
				...
				return Response.json(
					validateContractResponse(
						LegionDaemonApi.ControllerReady.response,
						{},
					),
				);
			}
			...
			if (pathname === "/legion/v1/gates/approve") {
				...
				await save();
				return Response.json(
					validateContractResponse(LegionDaemonApi.GatesApprove.response, {}),
				);
			}
			...
			if (pathname === "/legion/v1/backlog") {
				...
				await save();
				return Response.json(
					validateContractResponse(LegionDaemonApi.Backlog.response, {}),
				);
			}
	...
	return {
		...
		mintBootToken: async (tree, generation) => {
			...
			if (!treeState || treeState.generation !== generation) {
				throw new Error(
					`Cannot mint boot token for stale tree generation ${tree}`,
				);
			}

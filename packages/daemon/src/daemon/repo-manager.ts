...
export function resolveWorkspacePath(
	...
	const workspacePath = path.join(
		paths.forLegion(projectId).workspacesDir,
		issueId,
	);
	const resolvedPath = path.resolve(workspacePath);
	const expectedParent = path.resolve(paths.forLegion(projectId).workspacesDir);
	if (
		!resolvedPath.startsWith(expectedParent + path.sep) &&
		resolvedPath !== expectedParent
	) {
		throw new Error(
			`Workspace path would escape workspaces directory: ${resolvedPath}`,
		);
	}
	...
	if (!(await deps.exists(workspacePath))) {
		...
		if (result.exitCode !== 0) {
			...
			if (result.stderr.includes("already registered") && runGit) {
				...
				if (retry.exitCode !== 0) {
					throw new Error(
						`Failed to create workspace ${workspacePath}: ${retry.stderr}`,
					);
				}
			} else {
				throw new Error(
					`Failed to create workspace ${workspacePath}: ${result.stderr}`,
				);
			}
		}
	}
...
export async function startBackgroundFetch(
	...
	if (result.exitCode !== 0) {
		throw new Error(
			`Background fetch failed for ${clonePath}: ${result.stderr}`,
		);
	}
	return result;
}
...
export async function cleanupWorkspace(
	...
	if (!pushCheck.safe) {
		throw new Error(
			`Refusing to clean workspace for ${issueId}: ${pushCheck.reason}`,
		);
	}
	...
	if (runGit) {
		try {
			...
			if (pruneResult.exitCode !== 0) {
				console.warn(
					`[cleanup] git worktree prune failed (non-fatal): ${pruneResult.stderr}`,
				);
			}
		} catch {

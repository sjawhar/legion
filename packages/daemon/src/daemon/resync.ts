...
function boardIssue(item: Record<string, unknown>):
	...
	const status =
		typeof projectStatus === "string" ? projectStatus.toLowerCase() : "";
	return {
...
function openedBoardIssue(
	...
	const content = record(item.content);
	const repository = content?.repository;
	if (!content || typeof repository !== "string" || !projectId)
		return undefined;
...
export async function runResync(
	...
	const { items, excludedNullContentItems = 0 } =
		await deps.fetchGitHubProjectItems();
	...
	>) {
		if (
			...
			deps.state.issues[issue]?.backlogMarker
		)
			continue;
	...
	for (const item of items) {
		...
		if (issue) {
			...
			if (labels) {
				for (const label of new Set([...labels, ...issue.labels])) {
					...
					const effects = reduceGithubEvent(
						deps.state,
						"resync",
						envelope,
						deps.config,
				...
				anomalies.push(
					issue.released
						...
						: {
								...
								issue: board.issue,
								detail:
									"tracked open issue has no Legion tree or admission entry",
							},
				);
			} else if (
				issue.state === "open" &&
		...
		if (
			...
			continue;
		const envelope = openedBoardIssue(
			item,
			board.issue,
			deps.config.boardProjectIds[0],
			now,
		);
		if (envelope) {
			const effects = reduceGithubEvent(

...
function boardIssue(
  ...
  | { issue: IssueKey; repository: string; number: number; open: boolean; erroring: boolean }
  | undefined {
...
export async function runResync(deps: RunResyncDeps): Promise<LegionEventPayload> {
  ...
  return { type: "resync", anomalies, healed, reconciledLabels, excludedNullContentItems };
}

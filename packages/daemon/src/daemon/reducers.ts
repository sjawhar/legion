...
function addAttribution(
  state: LegionState,
  entry: { sha?: string; commentId?: number; sessionId: string; issue: IssueKey; phase: string }
): void {
  if (
...
function ingress(
  ...
  return [{ kind: "controller", payload: { type: "triage", issue: key, preexistingChildren } }];
}
...
function subIssue(
  ...
  if (payload.action === "sub_issue_added") {
    ...
    return route(
      ...
      parentKey,
      "architect",
      { type: "child-adopted", child: childKey, remaining: openChildren(state, parent) },
      envelope
    );
  }
  ...
  const result = route(
    ...
    parentKey,
    "architect",
    { type: "child-removed", child: childKey, remaining: openChildren(state, parent) },
    envelope
  );
...
function issueEvent(
  ...
  if (!tree) {
    return [
      {
        kind: "controller",
        payload: { type: "triage", issue: key, preexistingChildren: node.children },
      },
    ];
  }
...
export function reduceCiEmission(
  ...
  const envelope = { event_id: `ci:${repo}#${number}:${emission.sha}`, issued_at: pr.lastEventAt };

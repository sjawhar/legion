...
export type GitHubAppsConfig = Partial<
	Record<GitHubAppRole, GitHubAppRoleConfig>
>;
...
function executePrivateKeyCommand(command: string, field: string): string {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(
			`${field} failed (exit ${status})${stderr ? `: ${stderr}` : ""}`,
		);
	}
...
function loadGitHubApps(value: unknown): GitHubAppsConfig | undefined {
	...
	for (const role of ["implement", "review"] as const) {
		...
		const appId = readString(
			parsedRole.data.app_id,
			`github_apps.${role}.app_id`,
		);
		const inlineKey = readString(
			parsedRole.data.private_key,
			`github_apps.${role}.private_key`,
		);
		...
		if (inlineKey !== undefined && inlineKey !== "") {
			...
			privateKey = executePrivateKeyCommand(
				command,
				`github_apps.${role}.private_key_command`,
			);
		} else {
...
function parseGates(
	value: unknown,
	field: string,
): DaemonConfig["gates"] | undefined {
	const parsed = UnknownRecordSchema.safeParse(value);
	if (!parsed.success) throw new Error(`${field} must be a mapping`);
	const design =
		readString(parsed.data.design, `${field}.design`) ?? "root-issues";
...
export function loadConfigFromFile(
	...
	if (project !== undefined)
		fields.legionId = requireNonEmpty(project, "project");
	...
	if (envoyUrl !== undefined)
		fields.envoyUrl = validateUrl(envoyUrl, "envoy_url");
	...
	if (dispatchUrl !== undefined)
		fields.dispatchUrl = validateUrl(dispatchUrl, "dispatch_url");
	...
	const boardProjectIds = readStringArray(
		config.board_project_ids,
		"board_project_ids",
	);
	...
	if (stateDir !== undefined) {
		fields.stateDir = path.isAbsolute(stateDir)
			? stateDir
			: path.resolve(configDir, stateDir);
	}
...
export function resolveDaemonConfig(
	...
	if (!project)
		throw new Error(
			"LEGION_ID must include at least one alphanumeric character",
		);
	...
	if (!natsUrls.value || natsUrls.value.length === 0) {
		throw new Error(
			"ENVOY_NATS_URL is required (or set nats_urls in legion.yaml)",
		);
	}
	...
	if (!dispatchUrl.value || dispatchUrl.value.trim().length === 0) {
		throw new Error(
			"LEGION_DISPATCH_URL is required (or set dispatch_url in legion.yaml)",
		);
	}
	...
	const maxRecursionDepth = resolveValue(
		...
		parseEnvPositiveInteger(
			env.LEGION_MAX_RECURSION_DEPTH,
	...
	const maxFixAttempts = resolveValue(
		...
		parseEnvPositiveInteger(
			env.LEGION_MAX_FIX_ATTEMPTS,
			"LEGION_MAX_FIX_ATTEMPTS",
		),
	...
	const gates = resolveValue(
		opts.cliOverrides?.gates,
		fileGates(fields),
		undefined,
		{
			design: "root-issues",
	...
	return {
		config: {
			...
			ompInvocation: requireNonEmpty(
				ompInvocation.value,
				"LEGION_OMP_INVOCATION",
			),

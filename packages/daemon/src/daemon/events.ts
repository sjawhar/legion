...
const CHECK_TOPIC =
	/^notifications\.github\.([^.]+)\.([^.]+)\.pr\.(\d+)\.check(?:\.|$)/;
...
export interface EventPumpDeps {
	nats: {
		subscribe(
			subject: string,
			cb: (subject: string, data: string) => void,
		): () => void;
...
function checkInput(
	...
	const sha =
		stringValue(valueFrom(records, "sha")) ??
		stringValue(valueFrom(records, "head_sha"));
	const name =
	...
	const conclusion =
		rawConclusion === null || typeof rawConclusion === "string"
			...
			: undefined;
	const branch =
		stringValue(valueFrom(records, "branch")) ??
		stringValue(valueFrom(records, "head_branch"));
...
function issueForBranch(repo: string, branch: string): IssueKey | undefined {
	...
	if (!owner || !name || extra.length > 0 || !Number.isSafeInteger(number))
		return undefined;
...
function findOrCreatePr(
	...
	if (!branch) {
		const mapping = Object.entries(state.prByBranch).find(
			([branchKey, mappedPrKey]) =>
				mappedPrKey === prKey && branchKey.startsWith(`${input.repo}@`),
		);
...
function heldTarget(state: LegionState, token: string): HeldTarget | undefined {
	...
	return {
		...
		isActive: () =>
			state.issues[parsed.issue]?.released === true && tree.status === "active",
	};
}
...
function isMention(subject: string): boolean {
	return (
		(subject.startsWith("notifications.github.") ||
			subject.startsWith("notifications.slack.")) &&
...
export function startEventPump(deps: EventPumpDeps): EventPump {
	...
	const publishRole = async (
		...
		try {
			...
			const persisted = target
				? (held ?? addHeld(target, role, payloadJson, envelope))
				: undefined;
	...
	const applyEffects = async (
		...
		for (const effect of effects) {
			if (effect.kind === "publish")
				await publishEffect(effect.role, effect.payload, envelope);
	...
	const applyEffectsAndSave = async (
		effects: Effect[],
		envelope: EnvelopeJson,
	): Promise<void> => {
	...
	const settleChecks = async (): Promise<void> => {
		...
		for (const pr of Object.values(deps.state.prs)) {
			...
			await publishCiEmissions(
				pr,
				settle(pr, now, deps.config.ciQuietMs),
				envelope,
			);
		}
		await deps.saveState();
	...
	const handleMessage = async (
		...
		await deps.saveState();
		console.log(
			`[legion] consumed event ${envelope.event_id} subject=${subject}`,
		);
	};
	...
	return {
		...
		async drain(): Promise<void> {
			...
			if (failures.length > 0)
				throw new AggregateError(failures, "Event pump processing failed");
		},
		stop(): void {
			stopped = true;

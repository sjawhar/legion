...
function pluginPredates(installed: string, recorded: string): boolean {
  ...
  return (
    ...
    comparePluginVersion(recordedVersion, installedVersion) < 0
  );
}
    ...
    const workerImageProbe = verifyWorkerImage(
      {
        ...
        scheduling: kubernetes.scheduling,
  ...
  if (installedPluginVersion) {
    const live = [
      ...Object.entries(state.trees).flatMap(([issue, tree]) =>
        tree.locator ? [{ issue, role: "architect", locator: tree.locator }] : []
      ),
    ...
    for (const { issue, role, locator } of live) {
      ...
      if (version !== undefined && !pluginPredates(installedPluginVersion, version)) continue;
      const handle =
        locator.runtime === "tmux"
          ...
          : "external" in locator
            ...
            : `pod ${locator.podName}`;
      console.error(
        `[legion] live process ${issue} ${role} (${handle}) runs pi-legion-envoy ${version ?? "(unrecorded)"}; installed ${installedPluginVersion} — relaunch it (LEGION-164)`
      );
    }
  }

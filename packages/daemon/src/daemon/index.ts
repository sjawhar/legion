...
import {
	fetchGitHubProjectItems,
	type GitHubProjectItemsResult,
} from "../state/github-fetch";
...
async function publishToEnvoy(
	...
	if (!response.ok) {
		throw new Error(
			`Envoy publish to ${topic} failed with status ${response.status}`,
		);
	}
}
...
function defaultDependencies(config: DaemonConfig): DaemonDependencies {
	...
	return {
		...
		resolveDaemonEnvironment,
		statPrompt: stat,
		envoyPublish: (topic, payloadJson) =>
			publishToEnvoy(config, topic, payloadJson),
		fetchGitHubProjectItems: createBoardProjectItemsFetcher(
			board,
			tokenManager,
		),
		tokenManager,
...
export async function startDaemon(
	...
	const environment = await deps.resolveDaemonEnvironment(
		config.ompInvocation,
		{
			run: deps.runner,
		},
	);
	...
	const eventDeps: EventPumpDeps = {
		...
		onProbe: async (tree) => {
			if ((await processManager.probe(tree)) === "dead")
				await processManager.resurrect(tree);
		},
	...
	api = startLegionApi(
		{ port: config.port, hostname: "127.0.0.1", gates: config.gates },
		apiDeps,
	);
		...
		for (const tree of Object.values(state.trees)) {
			...
			if (Date.parse(tree.lingerUntil) <= now) {
				void processManager.expireLinger(tree.root).catch((error) => {
					console.error(
						`[legion] linger cleanup failed for ${tree.root}:`,
						error,
					);
				});
			}

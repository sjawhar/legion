...
function readInstallation(value: unknown): { owner: string; id: string } {
	if (!isRecord(value)) {
		throw new Error(
			"GitHub App installation discovery returned an invalid installation",
		);
	}
	...
	if (
		...
		throw new Error(
			"GitHub App installation discovery returned an invalid installation account",
		);
	}
...
export class TokenManager {
	...
	private readonly installationCache = new Map<
		GitHubAppRole,
		Map<string, string>
	>();
	private readonly missingInstallationCache = new Map<
		GitHubAppRole,
		Set<string>
	>();
	...
	async getToken(
		...
		if (!installationId) {
			throw new Error(
				`github_app_not_installed: ${role} not installed on ${owner}`,
			);
		}
		...
		const promise = this.generateToken(
			role,
			roleConfig,
			installationId,
			cacheKey,
	...
	private async resolveInstallationId(
		...
		const missingOwners =
			this.missingInstallationCache.get(role) ?? new Set<string>();
	...
	private async discoverInstallations(
		...
		for (let page = 1; ; page += 1) {
			...
			if (!Array.isArray(payload)) {
				throw new Error(
					"GitHub App installation discovery returned an invalid response",
				);
			}

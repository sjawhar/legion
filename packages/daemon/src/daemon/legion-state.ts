...
const ControllerRoleClaimSchema = z
	...
	.strict();
const RoleClaimSchema = z.union([
	WorkerRoleClaimSchema,
	ControllerRoleClaimSchema,
]);
...
const LegionStateSchema = z
	.object({
		...
		controllerLocator: z
			...
			.optional(),
		roles: z.record(
			z.string().regex(ENVOY_ROLE_TOKEN_PATTERN),
			RoleClaimSchema,
		),
		spawnCapabilities: z.record(
			z.string().regex(/^[a-f0-9]{64}$/),
			SpawnCapabilitySchema,
		),
		...
		try {
			...
			if (!hasErrnoCode(cleanupError, "ENOENT")) {
				throw new AggregateError(
					[error, cleanupError],
					`Failed to save Legion state to ${file}`,
				);
			}
		}
		throw error;

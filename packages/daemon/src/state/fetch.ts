...
export type OwnerCommandRunnerOptionsProvider = (
	owner: string,
 ...
 */
export function mapCiRollupState(
	state: string | null | undefined,
): CiStatusLiteral | null {
 ...
 */
export function mapMergeableState(
	state: string | null | undefined,
): MergeableStatusLiteral | null {
	if (state === null || state === undefined) {
		return null;
	}
	switch (state) {
...
export async function getPrReviewStateBatch(
	...
	if (!runnerOptionsForOwner) {
		if (runner === defaultRunner) {
			throw new Error(
				"Owner-scoped GitHub App runner options are required for GraphQL reads",
			);
		}
	...
	const batches = new Map<
		string,
		{ owner: string; refs: Record<string, GitHubPRRefType> }
	>();
	...
	for (let attempt = 0; attempt < maxAttempts; attempt++) {
		...
		for (const [repoAlias, [_owner, _repo]] of repoAliasMap) {
			...
			for (const [prAlias, [issueId]] of prAliases) {
				...
				const commits = rawPr.commits as
					| { nodes?: unknown[] }
					| null
					| undefined;

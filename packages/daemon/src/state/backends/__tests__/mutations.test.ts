...
describe("parseIssueIdParts", () => {
	...
	it("throws for invalid format (no number)", () => {
		expect(() => parseIssueIdParts("acme-backend")).toThrow(
			"Cannot parse issueId",
		);
	});
...
describe("LinearTracker.transitionIssue", () => {
	it("throws not-implemented error", async () => {
		...
		await expect(
			tracker.transitionIssue({ issueId: "ENG-42" }, "In Progress"),
...
describe("LinearTracker.removeLabel", () => {
	it("throws not-implemented error", async () => {
		const tracker = new LinearTracker();
		await expect(
			tracker.removeLabel({ issueId: "ENG-42" }, "unknown-label"),
		).rejects.toThrow("not implemented");
	});
});

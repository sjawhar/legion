...
export function parseIssueIdParts(issueId: string): {
	...
	if (numberIdx < 2) {
		throw new Error(
			`Cannot parse issueId "${issueId}" — expected format: owner-repo-number`,
		);
	}
...
export class GitHubTracker implements IssueTracker {
				...
				for (const ref of typedItem.blockerRefs) {
					if (
						...
						if (blockerOwnerRepo) {
							blockedByIds.push(
								buildIssueId(
									blockerOwnerRepo.owner,
									blockerOwnerRepo.repo,
									ref.number,
								),
							);
						}
	...
	private async gh(owner: string, args: string[]): Promise<string> {
		if (!this.github) {
			throw new Error(
				"GitHubTracker requires owner-scoped GitHub App runner options",
			);
		}
		return runGhCommand(
			args,
			this.github.runner,
			await this.github.runnerOptionsForOwner(owner),
		);
	}
	...
	private async graphql(
		...
		return JSON.parse(
			await this.gh(owner, ["api", "graphql", "-f", `query=${query}`]),
		);
	}
	...
	async transitionIssue(
		...
		const projectItems = issue?.projectItems as
			| Record<string, unknown>
			| undefined;
		const nodes = projectItems?.nodes as
			| Array<Record<string, unknown>>
			| undefined;

		if (!nodes || nodes.length === 0) {
			throw new Error(
				`transitionIssue: ${target.issueId} has no project items`,
			);
		}
		...
		if (!item) {
			throw new Error(
				`transitionIssue: ${target.issueId} has no project with Status field`,
			);
		}

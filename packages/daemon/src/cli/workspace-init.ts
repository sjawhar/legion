 ...
 * locations) beneath the env the caller supplies, the caller's winning:
 * `createProvisioningCredential`'s for the clone and the fetch (its doc comment says what that
 * env pins), `JJ_USER`/`JJ_EMAIL` for the adoption. `defaultRunner` spawns with exactly the env it
 * is given, and the caller's env alone has no PATH: on the daemon host `createDaemonRunner` merges
 * the resolved tool environment the same way; a pod has no such wrapper, so this is where `jj` and
 * `git` become resolvable. */

	...
	if (fs.existsSync(cacheFile)) {
		try {
			...
			if (keyUpper in legions) {
				...
				console.log(
					`Using cached: ${legionRef} → ${legion.name} (${legion.id})`,
				);
				return legion.id;
			}
		} catch {}
  ...
  `;
	...
	try {
		...
		try {
			...
			throw new Error(
				`Linear API returned non-JSON response (status ${response.status})`,
			);
		}
		const parsed = LinearTeamsResponseSchema.safeParse(jsonBody);
		if (!parsed.success) {
			throw new Error(
				`Linear API returned invalid response: ${parsed.error.message}`,
			);
		}

		if (parsed.data.errors && parsed.data.errors.length > 0) {
			throw new Error(
				parsed.data.errors[0]?.message ?? "Linear API returned GraphQL errors",
			);
		}
		...
		try {
			...
			console.warn(
				`Failed to write legion cache to ${cacheFile}: ${String(error)}`,
			);
		}

		const keyUpper = legionRef.toUpperCase();
		const legion = parsed.data.data.teams.nodes.find(
			(node) => node.key.toUpperCase() === keyUpper,
		);
		if (!legion) {

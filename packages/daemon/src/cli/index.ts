...
function daemonUrl(env: NodeJS.ProcessEnv, explicit?: string): string {
	return (
		...
		`http://127.0.0.1:${env.LEGION_DAEMON_PORT ?? "13370"}`
	);
}
...
export async function cmdGh(
	args: string[],
	deps: GhCommandDeps,
): Promise<void> {
	const response = await deps.fetch(
		`${daemonUrl(deps.env, deps.daemonUrl)}/legion/v1/gh-token`,
		{
			method: "POST",
	...
	const exitCode = await deps.spawnGh(
		args,
		buildGitHubTokenEnv(payload.data.token, deps.env),
	);
	if (exitCode !== 0)
		throw new CliError(`gh exited with status ${exitCode}`, exitCode);
}

export async function cmdCredential(
	...
	if (
		!credential.startsWith("username=") ||
		!credential.includes("\npassword=")
	) {
...
function requireHandoffPhase(value: unknown, name: string): HandoffPhase {
	if (!isHandoffPhase(value)) {
		throw new CliError(
			`Invalid phase for ${name}: expected one of ${HANDOFF_PHASES.join(", ")}`,
		);
	}
	return value;
}
...
function parseHandoffData(raw: string): Record<string, unknown> {
	...
	for (const field of ["schemaVersion", "phase", "completed"]) {
		if (field in parsed)
			throw new CliError(`Handoff data field ${field} is not allowed`);
	}
...
function loadStartConfig(
	...
	if (configPath) {
		...
		configFile = loadConfigFromFile(
			fs.readFileSync("legion.yaml", "utf8"),
			process.cwd(),
		);
	}
...
async function cmdStatus(project: string): Promise<void> {
	...
	const response = await fetch(
		`http://127.0.0.1:${entry.port}/legion/v1/state`,
	);
	if (!response.ok)
		throw new CliError(`Daemon for ${project} returned ${response.status}`);
	const state: unknown = await response.json();
	console.log(
		JSON.stringify(
			{ project, pid: entry.pid, port: entry.port, state },
			null,
			2,
		),
	);
...
async function cmdLegions(): Promise<void> {
	const paths = resolveLegionPaths(process.env, os.homedir());
	console.log(
		JSON.stringify(await readLegionsRegistry(paths.legionsFile), null, 2),
	);
}
...
function controllerSecret(): string {
	...
	if (!secret)
		throw new CliError(
			"LEGION_CONTROLLER_SECRET is required for controller commands",
		);
	return secret;
}
...
async function postController(
	...
	if (!response.ok)
		throw new CliError(
			`Daemon request failed (${response.status}): ${JSON.stringify(payload)}`,
		);
	return payload;
}
			...
			run: ({ args }) =>
				runCli(async () => {
					...
					const value = args.phase
						? readPhaseHandoff(
								workspace,
								requireHandoffPhase(args.phase, "phase"),
							)
		...
		messages: defineCommand({
			...
			run: ({ args }) =>
				runCli(async () => {
					console.log(
						JSON.stringify(
							readMessages(String(args.workspace ?? process.cwd())),
							null,
							2,
						),
					);
...
const restartCommand = defineCommand({
	...
	run: ({ args }) =>
		runCli(() =>
			cmdRestart(String(args.project), args.config as string | undefined),
		),
});
...
const ghCommand = defineCommand({
	...
	run: () =>
		runCli(() => {
			...
			if (args.length === 0)
				throw new CliError("Usage: legion gh -- <gh args…>");
...
const stateCommand = defineCommand({
	...
	run: () =>
		runCli(async () => {
			...
			if (!response.ok)
				throw new CliError(`Daemon request failed (${response.status})`);

...
# proof-sdk clone).
#
# CI (PROOF_EDITOR_DIST unset): clones sjawhar/proof-sdk at branch main into a
...
if [[ -n "${PROOF_EDITOR_DIST:-}" ]]; then
  ...
  src="$(mktemp -d)"
  trap 'rm -rf "$src"' EXIT
  git clone --depth 1 --branch main https://github.com/sjawhar/proof-sdk.git "$src"
  (cd "$src" && npm install && npm run build:lib)
fi
...
echo "prepared $dest from ${PROOF_EDITOR_DIST:-a fresh proof-sdk build}"

...
func (s *server) routes() []apiRoute {
	routes := []apiRoute{
		...
		{http.MethodPost, "/api/v1/issues", authAny, "Create an issue (native, or from a GitHub owner/repo#n ref).", s.createIssue},
		...
		{http.MethodGet, "/api/v1/inbox", authHuman, "Open asks waiting on the caller, grouped by whose turn it is.", s.listInbox},

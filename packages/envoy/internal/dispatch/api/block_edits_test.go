...
// Markdown ends a line at a carriage return, alone or before a line feed, and the browser editor
// reads it so, but this server's parser reads a carriage return that no line feed follows as
// text. A replace or a suggestion carrying one would store text the browser reads as other lines -
// `x\r---` as a heading, and code after it outside the block holding it - so it is refused, and
// the document is left as it was. A carriage return before a line feed is a line ending both
// parsers read the same way.
func TestDocumentEditsRefuseALoneCarriageReturn(t *testing.T) {
	var documentService *docs.Service
	handler, _ := newInteractionHandler(t, func(database *store.Store) docs.API {
		documentService = docs.New(docs.Deps{Store: database, Settle: time.Hour, MarkWait: 50 * time.Millisecond})
		t.Cleanup(func() { _ = documentService.Shutdown(context.Background()) })
	...
	for index, test := range []struct{ name, spec, with string }{
		{"text that the browser reads as a heading", paragraph, "x\r---"},
		{"text before a heading marker", paragraph, "x\r# y"},
		{"code in a blockquote", quoted, "a\rb"},
		{"at the end of code", quoted, "a\r"},
	} {
		t.Run(test.name, func(t *testing.T) {
			issue := createInteractionIssue(t, handler, "R"+string(rune('A'+index)), "carriage return", test.spec)
			before := text(issue.PrimaryArtifactID)
			edited := dispatchRequest(t, handler, http.MethodPost, "/api/v1/artifacts/"+issue.PrimaryArtifactID+"/edits", map[string]any{
				"ops": []map[string]any{{"op": "replace", "find": "Body.", "with": test.with}},
			}, "alice")
			if body := edited.Body.String(); edited.Code != http.StatusBadRequest || !strings.Contains(body, `"code":"INVALID_OP"`) || !strings.Contains(body, "carriage return") {
				t.Fatalf("replace: status=%d body=%s", edited.Code, body)
			}
			created := sessionRequest(t, handler, http.MethodPost, "/api/v1/issues/"+issue.Key+"/comments", map[string]any{
				"body": "suggest", "anchor": map[string]any{"artifact": "spec", "quote": "Body."},
			...
			comment := decodeBody[model.Comment](t, created)
			accepted := dispatchRequest(t, handler, http.MethodPost, "/api/v1/comments/"+comment.ID+"/accept", map[string]any{}, "alice")
			if body := accepted.Body.String(); accepted.Code != http.StatusBadRequest || !strings.Contains(body, "carriage return") {
				t.Fatalf("accept: status=%d body=%s", accepted.Code, body)
			}
			if after := text(issue.PrimaryArtifactID); after != before {
				t.Fatalf("after the refusals = %q, want it unchanged, %q", after, before)
			}
		})
	}
	issue := createInteractionIssue(t, handler, "RZ", "carriage return and line feed", quoted)
	edited := dispatchRequest(t, handler, http.MethodPost, "/api/v1/artifacts/"+issue.PrimaryArtifactID+"/edits", map[string]any{
		"ops": []map[string]any{{"op": "replace", "find": "Body.", "with": "a\r\nb"}},
	}, "alice")
	if edited.Code != http.StatusOK {
		t.Fatalf("replace with CR LF in code: status=%d body=%s", edited.Code, edited.Body.String())
	}
}

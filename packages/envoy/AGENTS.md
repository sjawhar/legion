...
- Every open ask read carries `waiting_on` (`human` | `agent`): the `turn` of the newest comment with that `ask_id`, `human` when nobody has replied (`attachWaitingOn`, `issue_queries.go`; the inbox and `GET /api/v1/asks/open` compute it inline in their newest-reply lateral join). It is on `GET /api/v1/inbox` rows, `GET /api/v1/asks/{id}`, `GET /api/v1/issues/{key}/asks`, `GET /api/v1/artifacts/{id}/asks`, and the issue detail's `open_asks`; closed asks and the `ask.*` event payloads (built from `loadAskForUpdate`) do not carry it. A `comment.created` payload that replies to an open ask carries the resulting `ask_waiting_on` so a stream consumer moves the ask without a refetch.

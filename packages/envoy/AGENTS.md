...
| `/v1/interests/subscribe` | POST | Persists session topics, route metadata, and optional delivery `capabilities`; registrations without capabilities persist `[]`. The response can include `warnings` when a GitHub repository has no retained events, and when a GitHub topic spells a repository name with its dot: the token after a topic's owner and name is always one of `contracts.GithubTopicKinds`, so a topic whose token there is not (`notifications.github.acme.site.io.pr.7.>`) is warned about, with the spelling Envoy publishes (`acme.site_io`). |

...
- Every open ask read carries `waiting_on` (`human` | `agent`): the `turn` of the newest comment with that `ask_id`, `human` when nobody has replied (`scanAskRead`, `api/ask_rows.go`; `GET /api/v1/asks/open` computes it inline in its own newest-reply lateral join). It is on `GET /api/v1/inbox` rows, `GET /api/v1/asks/{id}`, `GET /api/v1/issues/{key}/asks`, `GET /api/v1/artifacts/{id}/asks`, and the issue detail's `open_asks`; closed asks and the `ask.*` event payloads (built from `loadAskForUpdate`) do not carry it. A `comment.created` payload that replies to an open ask carries the resulting `ask_waiting_on` so a stream consumer moves the ask without a refetch.

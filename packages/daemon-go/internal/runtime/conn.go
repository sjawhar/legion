...
type Conn interface {
	...
	// refused it, or the transport failed. The turn it may start is observed on the stream.
	// The delivery id is the daemon's: an agent that receives the same id twice answers it
	// without starting a second turn.

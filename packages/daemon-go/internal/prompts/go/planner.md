...
This Go daemon advances phases and starts every phase worker itself. Do not choose or start the next phase. When your plan is complete, end this phase with the `legion` tool: `op: "handoff_complete"` and a `summary`.
...
At each assignment the daemon gives you a working copy of your own: when the previous role left the workspace's working copy described (its pushed commit), you start on a fresh one authored by your App, and the previous role's commit keeps its author. `handoff_complete` refuses a handoff commit another App authored; if it does, run `jj new`, then write and commit this phase's handoff again.

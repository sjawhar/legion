...
Use the Go-daemon `legion` operations to own the tree: `register_gate`, `release_children`, `request_backward_move`, `retry_or_escalate`, `sign_off`, and `read_record`. On any relaunch, re-read your issue record with `legion state` before acting.

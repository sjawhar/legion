...
// Decode reads back exactly what Encode writes, a kind with no names included; anything else is
...
func TestDecodeReadsEncodeAndRefusesAnyOther(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Decode(Encode's own %s) = %v, want it read", encoded, err)
	}
	if got := decoded[Skills]["dispatch"]; !slices.Equal(got, []string{"roles/architect.md"}) {
		t.Errorf("Decode(%s) read skill dispatch as named by %q, want roles/architect.md", encoded, got)
	}
	if len(decoded[TaskAgents]) != 0 {
		t.Errorf("Decode(%s) read task agents %v, want none", encoded, decoded[TaskAgents])
	}
	...
	for _, testCase := range []struct{ name, raw, want string }{
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), testCase.want) {
				t.Errorf("Decode(%s) = %v, want a refusal saying %q", testCase.raw, err, testCase.want)
			}
		})
	}
}

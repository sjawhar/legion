...
// The id a take-back replaced answers for the task only until a turn of that task's own confirms
// it. After that the agent is working it, and a refusal naming the replaced id — the retry the
// bound queued, refused because the agent is now busy with the very task it names — must not take
// the running turn's task back: the run the claim serves would drop to none mid-turn, the
// completion reported from that turn would be refused, the retire would never run, and the task
// would be prompted again.
func TestAConfirmedTurnEndsTheReplacedIdsClaimOnTheTask(t *testing.T) {
	h := newHarness(t)
	h.reach(StateReady)
	base := len(h.prompts())
	...
	// The task is re-sent under its new id and this turn is its own.
	resent := h.pending()
	h.must(StreamTurnStart{Claim: testToken, DeliveryID: resent.ID})
	if run := h.claim().ServingRun(); run != 7 {
		t.Fatalf("the claim serves run %d with its task's turn running, want 7", run)
	}

	// The refusal of the prompt the bound gave up on arrives now.
	h.must(StreamLateRefusal{Claim: testToken, DeliveryID: first.DeliveryID, Error: "Agent is already processing"})

	if run := h.claim().ServingRun(); run != 7 {
		t.Fatalf("the claim serves run %d after the stale refusal, want the running turn's 7", run)
	}
}

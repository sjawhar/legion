...
func TestEveryStreamEventMapsToItsSuperviseEvent(t *testing.T) {
	...
	cases := []struct {
		...
		{stream.LateRefusal{Claim: token, DeliveryID: "d2", Error: "Agent is already processing.", Replayed: true},
			supervise.StreamLateRefusal{Claim: token, DeliveryID: "d2", Error: "Agent is already processing.", Replayed: true}},

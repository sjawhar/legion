...
func TestOutboxTreeCloseRowEndsTheTreesRootClaim(t *testing.T) {
	...
	runner := &outbox{pool: pool, dispatchProject: "LEGION", records: records, supervisor: sup, tokens: outboxTokens{}, project: "legion", stateDir: t.TempDir(), repo: ghrepo.Repository{Owner: "acme", Name: "widgets"}}
...
func TestTheWorkflowsTreeCloseIsNotPutToTheOperatorsPredicate(t *testing.T) {
	...
	runner := &outbox{pool: pool, records: records, supervisor: sup, tokens: outboxTokens{}, project: "legion", stateDir: t.TempDir(), repo: ghrepo.Repository{Owner: "acme", Name: "widgets"}}

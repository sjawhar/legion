...
- `legion.daemonApiVersion` is 2. LEGION-20 (PR #975) introduced 2 for the `stateGate` and `GatesRegister` HTTP shapes — release 1.23.0 is the first to declare it — and LEGION-52 widens what the number covers: the pane contract, the environment variables the daemon sets on a pane that the extension reads or writes (`LEGION_GRANT_FILE`, `LEGION_BOOT_TOKEN_FILE`, `LEGION_CONTROLLER_SECRET_FILE`, `DISPATCH_TOKEN_FILE`, `DISPATCH_URL`, `LEGION_DAEMON_URL`, the `LEGION_*` identity variables), including the credential file (LEGION-54), as well as the daemon's HTTP request and response shapes. A change to either surface bumps the field and the daemon's `LEGION_DAEMON_API_VERSION` together in the same commit. A daemon at contract 2 refuses to start against any release through 1.22.2 (each declares 1, or nothing before LEGION-21) with `pi-legion-envoy at <manifest> (package <version>) speaks daemon API contract 1; this daemon requires 2`, instead of starting and spawning workers that fail at their first `legion gh`/`jj git push` because a pre-credential-file plugin never wrote the file (LEGION-52).

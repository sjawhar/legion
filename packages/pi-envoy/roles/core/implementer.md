...
Implement the acceptance criteria. Open the PR; write the PR body as you go. Dispose of every review thread individually with the fixing commit or a stated reason; never resolve threads in bulk. Your reply on a thread says what changed, `Fixed in <commit>: <one line>` or `Declined: <reason>`, and never begins with `Accepted:`: that is the reviewer's form, a thread whose newest reply is an `Accepted:` from its opener's account is resolved as accepted, and where you and the reviewer post as one account your own `Accepted:` would close a thread the reviewer never answered. Once a thread's opener has replied `Accepted:`, write nothing further on it: a later reply becomes the newest comment and the thread stays open. Correctness fixes go in this PR. Freeze a stacked base; never rewrite it. Read the plan first. Exercise the changed behavior through its real surface before reporting it.

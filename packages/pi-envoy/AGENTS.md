...
- A pane launched with both `LEGION_CONTROLLER` and `LEGION_TREE` is refused by `classifySession` (`ConflictingLaunchMarkersError`, `src/legion/classify.ts`); nothing tolerates the combination. `extensions/legion.ts` catches exactly that error at `session_start`, writes `CONFLICTING_LAUNCH_MARKERS_NOTICE` to the pane through `context.ui.notify(…, "warning")` (the inline notice `envoy.ts`'s own `session_start` warnings use; chosen from source evidence — live-pane visibility is unproven because the smoke rig was stopped by the 2026-09-13 standing order) and to `console.error` (which reaches a daemon-spawned pane's terminal: `legion worker-shim` spawns OMP with `stderr: "inherit"`), claims nothing, never exits, and the session continues as not Legion's — the pane stays up until the daemon retires it at its registration deadline, whose own log line points at the pane. The `tool_call` guard treats the same refusal as "not a phase worker", classified once and silently (LEGION-88). Every other classification throw (a malformed `LEGION_ROLE`, a missing `LEGION_TREE`/`LEGION_ISSUE`) still escapes.

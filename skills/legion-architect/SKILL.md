...
| `worker-recovered` | Payload `{type:"worker-recovered", issue, role, fromRef}`. The worker's tree volume was lost and it restarted as a new session from the committed handoff on `fromRef`. Re-send its current task; expect it to reconcile against `.legion/` first. |
